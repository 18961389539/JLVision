...
/// <para><b>转换口径</b>：本类只覆盖与 int 存储可互换的入口——<c>getI</c> 直读、<c>getL</c>/<c>getD</c> 拓宽返回（无精度损失）、
/// <c>getO</c> 逐元素装箱、<c>setI</c> 直写、<c>setL</c> 强转收窄（越界截断）；<c>getType</c> 恒报 <see cref="JlTupleType.INTEGER"/>，
/// 与基类默认行为不同。</para>
/// </remarks>
internal class JlTupleElementsInt32 : JlTupleElementsImplementation
{
	...
	/// </summary>
	/// <remarks>
	/// <para>底层把单个 <paramref name="index"/> 包装为长度为 1 的下标数组，等价于访问该元组的单一元素。</para>
	/// <para>本重载以单个 <paramref name="index"/> 定位元素；若需访问一组下标，请改用接收下标数组的重载，两者用途不同、不可混用。</para>
	/// </remarks>
