...
///   <para><b>构造与定位</b>：内部实现类，仅由 <c>JlTuple</c> 在装载 double 数据时创建——单值、<c>double[]</c>（是否拷贝由调用方决定）、
///   <c>float[]</c>（恒拷贝并逐元素拓宽为 double）三种来源；面向用户的读写一律走 <c>JlTuple</c> / <see cref="JlTupleElements"/>，不直接使用本类。</para>
///   <para><b>资源与坑</b>：<see cref="PinTuple"/> 用 <see cref="T:System.Runtime.InteropServices.GCHandle"/> 固定数组以便向原生层传指针，
///   因此调用方需在原生调用完成后调用 <c>UnpinTuple</c>（经 <c>JlNativeApi.UnpinTuple</c>），
///   否则数组无法被 GC 回收。</para>
/// </remarks>
