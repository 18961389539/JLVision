...
internal class JlTupleElementsDouble : JlTupleElementsImplementation
{
	...
	/// <summary>把给定 double 数组写入被选中下标；数组长度为 1 时广播到全部选中项，<c>indices</c> 为 null 时不写入。</summary>
	...
	/// <summary>读取被选中下标处的元素并逐个装箱为 object 数组；<c>indices</c> 为 null 时返回 <c>null</c>。</summary>
	...
	/// <summary>返回固定元素类型 <see cref="JlTupleType.DOUBLE"/>。</summary>
