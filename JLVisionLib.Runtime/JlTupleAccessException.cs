...
/// </summary>
/// <remarks>
///   <para><b>功能说明</b>：元组访问操作不合法时抛出，例如 <c>string</c>/<c>JlTupleElements</c>
///   下标越界、以与元组实际存储类型不符的方式取值（如对字符串元组取 <c>int</c>）、对未初始化或空元组取值等。</para>
///   <para><b>用法</b>：由元组各 <c>getX</c>/<c>setX</c> 的下标与类型校验路径抛出；因本类直接继承
///   <see cref="JlException"/>，外层按 <c>JlException</c> 兜底捕获即可，只有需要把“元组用法错误”与原生算子执行错误区分开时才单独 catch 本类型。</para>
