...
/// <para><b>约束或前提</b>：本类是 internal 视图基类，外部不会直接 new。真正的入口是 <see cref="JlTuple"/> 的下标器
/// （<c>this[int]</c> / <c>this[int[]]</c> / <c>this[JlTuple]</c>），它返回公开的 <see cref="JlTupleElements"/> 包装，
/// 后者再按底层元素的存储类型分派到 <see cref="JlTupleElementsInt32"/> 等子类。注意两套命名口径不同：本类成员
/// <c>I/L/D/S/H/O</c> 返回的都是数组，而公开 <see cref="JlTupleElements"/> 上 <c>I</c> 是标量、<c>IArr</c> 才是数组，勿混用。</para>
...
internal class JlTupleElementsImplementation
{
	...
	/// els.IArr = new[] { 15, 25 };   // 成组写入：下标0=15、下标2=25
	/// els.IArr = new[] { 7 };        // 广播：长度1，下标0、2 均写为 7
	/// int[] back = els.IArr;         // 成组读出 { 7, 7 }
	/// int first  = els.I;            // 标量读出：视图内第一个命中元素 = 7
	/// </code>
	/// </remarks>
	public int[] I
	{
