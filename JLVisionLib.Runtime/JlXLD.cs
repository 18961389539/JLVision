...
/// <summary>表示一个 XLD 对象（数组）的实例。</summary>
[Serializable]
public class JlXLD : JlObject, ISerializable, ICloneable
{
	...
	/// <summary>创建内部句柄为 UNDEF 的未初始化 JlXLD 容器，作为 Deserialize、Clone 等装载句柄前的占位；未初始化容器不能作为算子的图标输入。</summary>
	/// <remarks>
	...
	/// <summary>把已有原生轮廓句柄包成托管 JlXLD：执行引用计数拷贝（CopyObject），不是轮廓数据深拷贝；随后断言原生对象类为 xld。</summary>
	...
	/// <summary>按原生句柄构造轮廓容器，copy 决定是"CopyObject 加引用"还是"直接接管裸句柄"；随后断言对象类为 xld。</summary>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>把任意 JlObject 包装/复制成轮廓容器：对其句柄做引用计数拷贝并做原生类名校验，非 xld 类型抛 JlException。</summary>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>把原生过程某槽位的输出句柄装载进全新 JlXLD（"返回新句柄"路径的统一出口），透传错误码、自身不抛异常。</summary>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>ISerializable 反序列化构造器：从 info 的 "data" 键取出字节块，经 DeserializeXld（原生 id 1552）原地重建轮廓容器。</summary>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>把当前容器序列化为二进制写入 stream（内部先经 SerializeXld 得到字节缓冲再写流）；不关闭传入流，不产生新句柄。</summary>
	/// <remarks>
	...
	/// <summary>静态方法：从 Serialize 写出的二进制流读回一个新的 JlXLD 容器（返回新句柄，需 Dispose）。</summary>
	/// <remarks>
	...
	/// <summary>把容器中的轮廓按"平行线段对"读出 P1/P2 两侧线段的端点行、列、长度与法向角，共 8 组 DOUBLE out 元组；原地读取，不产生新句柄。</summary>
	/// <param name="row1">多边形 P1 上各点的行坐标。</param>
	/// <param name="col1">多边形 P1 上各点的列坐标。</param>
	/// <param name="length1">多边形 P1 上线段的长度。</param>
	/// <param name="phi1">与多边形 P1 上线段法向量所成的角度。</param>
	/// <param name="row2">多边形 P2 上各点的行坐标。</param>
	/// <param name="col2">多边形 P2 上各点的列坐标。</param>
	/// <param name="length2">多边形 P2 上线段的长度。</param>
	...
	/// <summary>按内容逐对判等，从本容器剔除与 objectsSub 内容相同的轮廓，返回剩余元素组成的新句柄容器；与顺序无关，两输入容器均不改动。</summary>
	...
	/// <summary>把容器中的轮廓按逐条给定的灰度描画到 image 的副本上，返回新图像句柄；输入图像不改动。</summary>
	/// <param name="image">xld 对象绘制到的图像。</param>
	/// <param name="grayval">xld 对象期望的灰度值。Default: 255.0</param>
	...
	/// <summary>PaintXld(JlImage, JlTuple) 的标量重载：同一灰度用于全部轮廓，描画到 image 的副本上返回新图像句柄；输入图像不改动。</summary>
	/// <param name="image">xld 对象绘制到的图像。</param>
	/// <param name="grayval">xld 对象期望的灰度值。Default: 255.0</param>
	/// <returns>包含结果的图像。</returns>
	/// <remarks>
	...
	/// <summary>复制本容器中自 index 起的 numObj 条轮廓为新句柄容器；取连续区段，原容器不改动。</summary>
	/// <param name="index">待复制对象的起始索引。Default: 1</param>
	/// <param name="numObj">要复制的对象数量或 -1。Default: 1</param>
	...
	/// <summary>把 objects2 的轮廓接到本容器之后（this 在前、objects2 在后），返回新句柄容器；两个输入容器均不改动。</summary>
	...
	/// <summary>按 index 中的序号（可含多个、可重复）把选中的轮廓装入新句柄容器；本容器不改动，序号语义依赖上游输出顺序。</summary>
	/// <param name="index">要选择的对象索引。Default: 1</param>
	...
	/// <summary>SelectObj(JlTuple) 的标量重载：按单个序号只取一条轮廓，返回新句柄容器；语义与坑见元组主重载。</summary>
	/// <param name="index">要选择的对象索引。Default: 1</param>
	/// <returns>筛选出的对象。</returns>
	/// <remarks>
	...
	/// <summary>带容差 epsilon 逐对比较两容器轮廓内容是否相等，以 int 返回布尔判定；不产生新句柄，epsilon 为 0 时逐坐标精确比较。</summary>
	/// <param name="objects2">待测试的对象。</param>
	/// <param name="epsilon">两个灰度值或坐标等之间的最大允许偏差。Default: 0.0</param>
	...
	/// <summary>CompareObj(JlXLD, JlTuple) 的标量重载：整个比较使用同一容差，以 int 返回布尔判定，不产生新句柄；语义与坑见元组主重载。</summary>
	/// <param name="objects2">待测试的对象。</param>
	/// <param name="epsilon">两个灰度值或坐标等之间的最大允许偏差。Default: 0.0</param>
	/// <returns>布尔结果值。</returns>
	/// <remarks>
	...
	/// <summary>不带容差地判断两个轮廓容器是否逐元素相等，以 int 返回布尔判定；不产生新句柄，拟合/近似结果的比对应改用 CompareObj。</summary>
	...
	/// <summary>基于规则网格点 row/column 计算畸变图与校正图之间的映射：返回新映射图像句柄，并把网格线轮廓 out 为新容器 meshes；两者均需 Dispose。</summary>
	/// <param name="image">输入图像。</param>
	/// <param name="meshes">输出轮廓。</param>
	/// <param name="gridSpacing">校正图像中网格点的间距。</param>
	/// <param name="rotation">施加于点网格的旋转。Default: "auto"</param>
	...
	/// <summary>GenGridRectificationMap 主实现的字符串重载：rotation 以单一控制串传入，返回新映射图像句柄并 out 出新网格容器 meshes；语义与坑见 (JlTuple rotation) 主重载。</summary>
	/// <param name="image">输入图像。</param>
	/// <param name="meshes">输出轮廓。</param>
	/// <param name="gridSpacing">校正图像中网格点的间距。</param>
	/// <param name="rotation">施加于点网格的旋转。Default: "auto"</param>
	...
	/// <summary>从 SerializeXld 得到的字节缓冲原地恢复本容器的轮廓：先 Dispose 旧句柄再装载，本容器内容被直接覆盖，不产生新句柄。</summary>
	/// <param name="serializedItemHandle">序列化数据项的句柄。</param>
	/// <remarks>
	...
	/// <summary>把本容器序列化为字节数组（DeserializeXld 的逆操作），返回序列化项的托管 byte[] 拷贝而非句柄。</summary>
	/// <returns>序列化数据项的句柄。</returns>
	/// <remarks>
	...
	/// <summary>逐条判断轮廓/多边形是否闭合，返回 1 闭合、0 开放的 INTEGER 元组，长度等于容器内轮廓数；只给判定不筛容器。</summary>
	...
	/// <summary>把轮廓/多边形当作点集求任意阶几何矩 M(p,q)（点集口径、与轮廓方向无关），每条轮廓一组值，返回 DOUBLE 元组，无句柄产生。</summary>
	/// <param name="mode">计算模式。Default: "unnormalized"</param>
	/// <param name="area">轮廓或多边形所围区域的面积。</param>
	/// <param name="centerRow">质心的行坐标。</param>
	/// <param name="centerCol">质心的列坐标。</param>
	/// <param name="p">所需矩 M[P,Q]@f$M_{p,q}$ 的第一索引。Default: 1</param>
	/// <param name="q">所需矩 M[P,Q]@f$M_{p,q}$ 的第二索引。Default: 1</param>
	...
	/// <summary>MomentsAnyPointsXld 元组版的标量重载：单条轮廓求单值矩，结果经 LoadD 以 double 取回；语义与坑见元组主重载。</summary>
	/// <param name="mode">计算模式。Default: "unnormalized"</param>
	/// <param name="area">轮廓或多边形所围区域的面积。</param>
	/// <param name="centerRow">质心的行坐标。</param>
	/// <param name="centerCol">质心的列坐标。</param>
	...
	/// <summary>点集口径求长细度（等价椭圆长半轴/短半轴 ra/rb 之比），每条轮廓一个 DOUBLE 值返回元组；要 ra/rb/phi 全套参数改用 EllipticAxisPointsXld。</summary>
	...
	/// <summary>点集口径求等价椭圆参数：返回长半轴 ra 元组，out 出短半轴 rb 与主轴方向 phi（弧度），每条轮廓各一组 DOUBLE 值；开口轮廓同样按点集处理。</summary>
	/// <param name="rb">短半轴。</param>
	/// <param name="phi">主轴与列轴的夹角（弧度）。</param>
	/// <returns>长半轴。</returns>
	/// <remarks>
	...
	/// <summary>EllipticAxisPointsXld 元组版的标量重载：单条轮廓取 ra/rb/phi（弧度）单值，结果经 LoadD 以 double 装载；语义与坑见元组主重载。</summary>
	/// <param name="rb">短半轴。</param>
	/// <param name="phi">主轴与列轴的夹角（弧度）。</param>
	/// <returns>长半轴。</returns>
	/// <remarks>
	...
	/// <summary>点集口径求主方向角（由二阶矩导出，弧度），每条轮廓一个 DOUBLE 值返回元组；区域口径改用 OrientationXld。</summary>
	...
	/// <summary>点集口径求二阶矩：返回混合矩 m11，out 出行向 m20 与列向 m02，均为 DOUBLE 元组、每条轮廓一组值；与轮廓方向无关。</summary>
	/// <param name="m20">沿行轴的二阶矩。</param>
	...
	/// <summary>MomentsPointsXld 元组版的标量重载：单条轮廓取 m11/m20/m02 单值，结果经 LoadD 以 double 装载；语义与坑见元组主重载。</summary>
	/// <param name="m20">沿行轴的二阶矩。</param>
	/// <param name="m02">沿列轴的二阶矩。</param>
	/// <returns>混合二阶矩。</returns>
	/// <remarks>
	...
	/// <summary>把轮廓/多边形当作点集求"面积"与算术质心（点集口径，非闭合区域几何面积，开放轮廓同样成立）：返回 DOUBLE 元组面积，out 出质心 row/column。</summary>
	/// <param name="row">质心的行坐标。</param>
	...
	/// <summary>AreaCenterPointsXld 元组版的标量重载：单条轮廓取"面积"与质心 row/column 单值，结果经 LoadD 以 double 装载；语义与坑见元组主重载。</summary>
	/// <param name="row">质心的行坐标。</param>
	/// <param name="column">质心的列坐标。</param>
	/// <returns>点集的面积。</returns>
	/// <remarks>
	...
	/// <summary>逐条判断轮廓/多边形是否几何自交（如打结轮廓），closeXLD 控制检测前是否先虚拟闭合开放轮廓，返回 1 自交、0 否的 INTEGER 元组。</summary>
	/// <param name="closeXLD">是否先闭合输入的轮廓或多边形？Default: "true"</param>
	/// <returns>轮廓或多边形存在自相交时为 1，否则为 0。</returns>
	/// <remarks>
	...
	/// <summary>从容器中选出恰好经过测试点 (row,column) 的轮廓子集（"经过"判定，非内外包含），返回新句柄容器；本容器不改动。</summary>
	...
	/// <summary>SelectXldPoint(JlTuple, JlTuple) 的标量重载：全容器使用同一个测试点，返回命中轮廓的新句柄容器；语义与坑见元组主重载。</summary>
	/// <param name="row">测试点的行坐标。Default: 100.0</param>
	/// <param name="column">测试点的列坐标。Default: 100.0</param>
	/// <returns>包含该测试点的全部轮廓或多边形。</returns>
	/// <remarks>
	...
	/// <summary>判断给定点是否被闭合轮廓/多边形包围（内外关系），返回 1 在内、0 在外的 INTEGER 布尔元组；开放轮廓的包围判定不可靠。</summary>
	/// <param name="row">待测试点的行坐标。</param>
	/// <param name="column">待测试点的列坐标。</param>
	/// <returns>布尔数值元组。</returns>
	/// <remarks>
	...
	/// <summary>TestXldPoint(JlTuple, JlTuple) 的标量重载：单个测试点、经 LoadI 返回单个 int 判定（1 在内、0 在外）；语义与坑见元组主重载。</summary>
	/// <param name="row">待测试点的行坐标。</param>
	/// <param name="column">待测试点的列坐标。</param>
	/// <returns>布尔数值元组。</returns>
	/// <remarks>
	...
	/// <summary>按形状特征值区间筛选轮廓（min/max 可用字符串表示不设界，多特征以 operation 组合），返回满足条件的新句柄容器；本容器不改动，按特征选取不随上游输出顺序静默错位。</summary>
	/// <param name="features">要检查的形状特征。Default: "area"</param>
	/// <param name="operation">各特征之间的运算类型。Default: "and"</param>
	/// <param name="min">各特征的下限值（或填 'min'）。Default: 150.0</param>
	/// <param name="max">各特征的上限值（或填 'max'）。Default: 99999.0</param>
	...
	/// <summary>SelectShapeXld 元组版的标量重载：单特征、单一区间，返回满足条件的新句柄容器；语义与坑见元组主重载。</summary>
	/// <param name="features">要检查的形状特征。Default: "area"</param>
	/// <param name="operation">各特征之间的运算类型。Default: "and"</param>
	/// <param name="min">各特征的下限值（或填 'min'）。Default: 150.0</param>
	/// <param name="max">各特征的上限值（或填 'max'）。Default: 99999.0</param>
	...
	/// <summary>按闭合轮廓所围区域面积的矩求主方向角（弧度），每条轮廓一个 DOUBLE 值返回元组；点集口径改用 OrientationPointsXld。</summary>
	/// <returns>轮廓或多边形的方向（弧度）。</returns>
	/// <remarks>
	...
	/// <summary>由等价椭圆半轴导出形状因子组（区域口径）：返回长细度 anisometry，out 出丰度 bulkiness 与结构因子 structureFactor，每条轮廓一组 DOUBLE 值。</summary>
	/// <param name="bulkiness">轮廓或多边形的臃肿度。</param>
	...
	/// <summary>EccentricityXld 元组版的标量重载：单条轮廓取三值单值；容器含多条轮廓时 LoadD 只取第 1 条结果、其余静默丢弃。</summary>
	/// <param name="bulkiness">轮廓或多边形的臃肿度。</param>
	/// <param name="structureFactor">轮廓或多边形的结构因子。</param>
	/// <returns>轮廓或多边形的各向异性。</returns>
	/// <remarks>
	...
	/// <summary>求衡量轮廓接近圆程度的紧凑度纯数因子，每条轮廓一个 DOUBLE 值返回元组；与 CircularityXld、ConvexityXld 口径不同，不要互换阈值。</summary>
	...
	/// <summary>逐条轮廓求点集最大间距（直径）：out 出两个极值点的 row/column 与二者距离 diameter，共 5 组 DOUBLE 元组；极值在轮廓顶点集中搜索。</summary>
	/// <param name="row1">轮廓或多边形第一个极端点的行坐标。</param>
	/// <param name="column1">轮廓或多边形第一个极端点的列坐标。</param>
	/// <param name="row2">轮廓或多边形第二个极端点的行坐标。</param>
	/// <param name="column2">轮廓或多边形第二个极端点的列坐标。</param>
	...
	/// <summary>DiameterXld 元组版的标量重载：单条轮廓 out 出 5 个 double 单值；容器含多条轮廓时只取第 1 条结果、其余静默丢弃。</summary>
	/// <param name="row1">轮廓或多边形第一个极端点的行坐标。</param>
	/// <param name="column1">轮廓或多边形第一个极端点的列坐标。</param>
	/// <param name="row2">轮廓或多边形第二个极端点的行坐标。</param>
	/// <param name="column2">轮廓或多边形第二个极端点的列坐标。</param>
	...
	/// <summary>
	///   逐条轮廓求等价椭圆参数：返回长半轴 ra（DOUBLE 元组），out 短半轴 rb 与主轴方向 phi（弧度，主轴与 x 轴即 column 轴的夹角）。
	/// </summary>
	/// <param name="rb">短半轴。</param>
	/// <param name="phi">主轴与 x 轴的夹角（弧度）。</param>
	/// <returns>长半轴。</returns>
	/// <remarks>
	...
	/// <summary>
	///   元组版的标量重载：返回单条轮廓等价椭圆的长半轴 ra，out 短半轴 rb 与主轴方向 phi（弧度）；容器含多条轮廓时只取第 1 条的结果，其余静默丢弃。
	/// </summary>
	/// <param name="rb">短半轴。</param>
	/// <param name="phi">主轴与 x 轴的夹角（弧度）。</param>
	/// <returns>长半轴。</returns>
	...
	/// <summary>
	///   逐条轮廓求任意方向最小面积外接矩形：out 中心 row/column、方向 phi（弧度）与两条半边长 length1/length2（DOUBLE 元组，整边长需乘 2）。
	/// </summary>
	/// <param name="row">外接矩形中心的行坐标。</param>
	/// <param name="column">外接矩形中心的列坐标。</param>
	/// <param name="phi">外接矩形的方向（弧度）。</param>
	...
	/// <summary>
	///   元组版的标量重载：单条轮廓任意方向最小外接矩形的中心 row/column、方向 phi（弧度）与两条半边长；容器含多条轮廓时只取第 1 条的结果，其余静默丢弃。
	/// </summary>
	/// <param name="row">外接矩形中心的行坐标。</param>
	/// <param name="column">外接矩形中心的列坐标。</param>
	/// <param name="phi">外接矩形的方向（弧度）。</param>
	...
	/// <summary>
	///   逐条轮廓求平行于坐标轴的最小外接矩形：out 左上角 (row1, column1) 与右下角 (row2, column2)，即顶点集的行列极值（4 组 DOUBLE 元组）。
	/// </summary>
	/// <param name="row1">外接矩形左上角点的行坐标。</param>
	/// <param name="column1">外接矩形左上角点的列坐标。</param>
	/// <param name="row2">外接矩形右下角点的行坐标。</param>
	...
	/// <summary>
	///   元组版的标量重载：单条轮廓轴向外接矩形的左上角 (row1, column1) 与右下角 (row2, column2)；容器含多条轮廓时只取第 1 条的结果，其余静默丢弃。
	/// </summary>
	/// <param name="row1">外接矩形左上角点的行坐标。</param>
	/// <param name="column1">外接矩形左上角点的列坐标。</param>
	/// <param name="row2">外接矩形右下角点的行坐标。</param>
	...
	/// <summary>
	///   逐条轮廓求覆盖全部顶点的最小外接圆：out 圆心 row/column 与半径 radius（像素），3 组 DOUBLE 元组。
	/// </summary>
	/// <param name="row">外接圆圆心的行坐标。</param>
	/// <param name="column">外接圆圆心的列坐标。</param>
	/// <param name="radius">外接圆的半径。</param>
	/// <remarks>
	...
	/// <summary>
	///   元组版的标量重载：单条轮廓最小外接圆的圆心 row/column 与半径 radius（像素）；容器含多条轮廓时只取第 1 条的结果，其余静默丢弃。
	/// </summary>
	/// <param name="row">外接圆圆心的行坐标。</param>
	/// <param name="column">外接圆圆心的列坐标。</param>
	/// <param name="radius">外接圆的半径。</param>
	...
	/// <summary>
	///   按 type 对每条轮廓做形状变换（确证取值 "convex"：由顶点集生成凸包轮廓），返回新 JlXLD 句柄需 Dispose，本容器不改动。
	/// </summary>
	...
	/// <summary>
	///   逐条轮廓求折线长度（像素，相邻顶点欧氏距离之和）：返回与轮廓数等长的 DOUBLE 元组。
	/// </summary>
	...
	/// <summary>
	///   按所围区域口径计算每条轮廓的任意阶几何矩 M(p,q)，返回 DOUBLE 元组；须配套传入本容器 AreaCenterXld 输出的面积、质心与顶点绕行方向。
	/// </summary>
	/// <param name="mode">计算模式。Default: "unnormalized"</param>
	/// <param name="pointOrder">沿边界的顶点绕行方向。Default: "positive"</param>
	/// <param name="area">轮廓或多边形所围区域的面积。</param>
	/// <param name="centerRow">质心的行坐标。</param>
	/// <param name="centerCol">质心的列坐标。</param>
	/// <param name="p">所需矩 M[P,Q]@f$M_{p,q}$ 的第一索引。Default: 1</param>
	...
	/// <summary>
	///   元组版的标量重载：按所围区域口径求单条轮廓的任意阶几何矩 M(p,q)（double）；容器含多条轮廓时只取第 1 条的结果，其余静默丢弃。
	/// </summary>
	/// <param name="mode">计算模式。Default: "unnormalized"</param>
	/// <param name="pointOrder">沿边界的顶点绕行方向。Default: "positive"</param>
	/// <param name="area">轮廓或多边形所围区域的面积。</param>
	...
	/// <summary>
	///   按所围区域口径求每条轮廓的二阶几何矩：返回混合矩 m11，out 行向 m20 与列向 m02，均为 DOUBLE 元组、每条轮廓一组，无需外部参考量。
	/// </summary>
	/// <param name="m20">沿行轴的二阶矩。</param>
	/// <param name="m02">沿列轴的二阶矩。</param>
	/// <returns>混合二阶矩。</returns>
	...
	/// <summary>
	///   元组版的标量重载：单条轮廓区域口径二阶矩——返回混合矩 m11，out 行向 m20 与列向 m02；容器含多条轮廓时只取第 1 条的结果，其余静默丢弃。
	/// </summary>
	/// <param name="m20">沿行轴的二阶矩。</param>
	/// <param name="m02">沿列轴的二阶矩。</param>
	/// <returns>混合二阶矩。</returns>
	...
	/// <summary>
	///   逐条闭合轮廓求所围面积与质心：返回 area（DOUBLE 元组），out 质心 row/column 与 pointOrder（字符串元组，报告该轮廓实际的顶点绕行方向）。
	/// </summary>
	/// <param name="row">质心的行坐标。</param>
	/// <param name="column">质心的列坐标。</param>
	/// <param name="pointOrder">沿边界的顶点绕行方向（'positive'/'negative'）。</param>
	...
	/// <summary>
	///   元组版的标量重载：单条闭合轮廓的所围面积（返回）、质心 row/column 与顶点绕行方向 pointOrder（string）；容器含多条轮廓时只取第 1 条的结果，其余静默丢弃。
	/// </summary>
	/// <param name="row">质心的行坐标。</param>
	/// <param name="column">质心的列坐标。</param>
	/// <param name="pointOrder">沿边界的顶点绕行方向（'positive'/'negative'）。</param>
	...
	/// <summary>
	///   逐条轮廓求轴向外接矩形的尺寸：返回高（row 方向边长，像素），out 宽（column 方向边长）与长宽比，均为 DOUBLE 元组。
	/// </summary>
	/// <param name="width">外接矩形的宽度。</param>
	...
	/// <summary>
	///   元组版的标量重载：单条轮廓轴向外接矩形的高（返回）、宽与长宽比；容器含多条轮廓时只取第 1 条的结果，其余静默丢弃。
	/// </summary>
	/// <param name="width">外接矩形的宽度。</param>
	/// <param name="ratio">外接矩形的长宽比。</param>
	/// <returns>外接矩形的高度。</returns>
	...
	/// <summary>
	///   把 objectsInsert 的全部轮廓插入本容器第 index 个位置之前，原元素整体后移，返回扩展后的新 JlXLD 句柄需 Dispose；本容器与 objectsInsert 均不改动。
	/// </summary>
	/// <param name="objectsInsert">待插入的对象元组。</param>
	/// <param name="index">插入对象的位置索引。</param>
	...
	/// <summary>
	///   按序号 index 把指定轮廓从本容器剔除，其余按原顺序装入新 JlXLD 句柄返回需 Dispose；本容器不改动。
	/// </summary>
	/// <param name="index">要移除的对象索引。</param>
	...
	/// <summary>
	///   标量重载：按单个序号剔除一条轮廓，其余按原顺序装入新 JlXLD 句柄返回需 Dispose；剔除后剩余轮廓序号整体前移。
	/// </summary>
	/// <param name="index">要移除的对象索引。</param>
	/// <returns>剩余的对象元组。</returns>
	/// <remarks>
	...
	/// <summary>
	///   用 objectsReplace 中的轮廓按位替换本容器第 index 个元素（原位覆盖，元素总数不变），返回新 JlXLD 句柄需 Dispose；本容器与 objectsReplace 均不改动。
	/// </summary>
	/// <param name="objectsReplace">用于替换的元素。</param>
	/// <param name="index">待替换元素的索引。</param>
	...
	/// <summary>
	///   元组版的标量重载：按单个序号原位替换一处轮廓，容器长度不变，返回新 JlXLD 句柄需 Dispose。
	/// </summary>
	/// <param name="objectsReplace">用于替换的元素。</param>
	/// <param name="index">待替换元素的索引。</param>
	/// <returns>替换元素后的元组。</returns>
	...
	/// <summary>
	///   每条轮廓给出一个矩形度因子（DOUBLE 元组）：衡量轮廓与矩形的相似程度，越接近规则矩形值越大。
	/// </summary>
