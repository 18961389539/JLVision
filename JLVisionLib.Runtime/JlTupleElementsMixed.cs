...
///   </para>
///   <para>
///     <b>取舍</b>：相比同构实现，本形式以每元素一次 <c>object[]</c> 装箱为代价，换取同一元组各下标自由混存
///     int/long/double/string/JlHandle 的能力；若整段数据同型，应交给同构实现承载，只有下标间类型确实不一致时才落到本类。
///   </para>
///   <para>
...
internal class JlTupleElementsMixed : JlTupleElementsImplementation
{
	...
	///   <para><b>广播规则</b>：同 <see cref="setI"/>（长度 1 广播，否则须等于下标个数，null 忽略）。</para>
	///   <para>
	///     <b>用法</b>：<c>setI</c>/<c>setL</c>/<c>setD</c>/<c>setS</c>/<c>setH</c> 都要求整批元素统一为一个类型，
	///     只有本入口允许数组内各元素各自持有类型：既用于一次铺写异构片段，也可直接消费另一元组 <c>getO</c> 的结果做整段拷贝。
	///   </para>
	///   <para>
