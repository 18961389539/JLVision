...
/// <summary>表示一个区域对象（或区域元组）的实例。</summary>
[Serializable]
public class JlRegion : JlObject, ISerializable, ICloneable
{
	...
	/// <summary>由原生图标句柄包出区域实例，并在原生侧再深拷贝一份（copy_obj，id 568）：新实例与传入的 key 各自独立。</summary>
	/// <param name="key">原生图标句柄，取自 JlObjectBase.Key 或 CopyKey() 一类出口。</param>
	/// <remarks>
	///   <para><b>功能说明</b>转 <c>this(key, copy: true)</c>，即 <c>JlNativeApi.CopyObject(key)</c>（原生 id 568：控制参数 <c>StoreI(0,1)</c> 表示拷 1 份、<c>StoreI(1,-1)</c> 的 -1 表示整条区域元组全取），随后对本实例执行 <c>AssertObjectClass</c>。</para>
	///   <para><b>约束或前提</b><c>key</c> 等于 <c>JlObjectBase.UNDEF</c> 时基类的拷贝分支被跳过，得到未初始化实例；类名校验经 <c>get_object_class</c>（id 579），名字既不以 region 开头也不是 any 的句柄直接抛 <c>JlException</c>。</para>
	///   <para><b>与相邻成员的取舍</b>不想付这次原生拷贝、要让托管实例接管 key 的释放责任时用 <see cref="JlRegion(IntPtr,bool)"/>；手上已经是托管对象时用 <see cref="JlRegion(JlObject)"/>。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   JlRegion src = new JlRegion(10.0, 10.0, 30.0, 30.0);
	///   System.IntPtr key = src.CopyKey();                 // CopyKey 交出的 key 由调用方负责释放
	///   JlRegion dup = new JlRegion(key);                  // 构造内再拷一份，dup 与 key 互不影响
	///   JlRegion owner = new JlRegion(key, copy: false);   // 只为给多余的 key 一个托管归宿
	///   owner.Dispose();
	///   dup.Dispose();
	///   src.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>本构造只释放自己那份拷贝，不会释放传进来的 key；先 Dispose 哪个实例都不影响另一个。</para>
	/// </remarks>
	...
	/// <summary>用原生句柄建区域实例，由 copy 决定再深拷贝一次还是直接把 key 接管过来由本实例释放。</summary>
	/// <param name="key">原生图标句柄。</param>
	/// <param name="copy">true 时走原生 copy_obj（id 568）取独立副本；false 时直接收下 key，本实例成为它的唯一释放者。</param>
	/// <remarks>
	///   <para><b>功能说明</b>基类判定 <c>copy &amp;&amp; key != UNDEF &amp;&amp; key != UNDEF2</c> 才调 <c>JlNativeApi.CopyObject</c>，否则 <c>this.key = key</c>（内部哨兵 UNDEF2 归一为 UNDEF）；收尾 <c>AssertObjectClass</c> 经 id 579 确认该句柄确实是 region。</para>
	///   <para><b>约束或前提</b>copy=false 时不产生任何原生调用，实例与 key 是同一个句柄：<c>Dispose</c> 会执行 <c>ClearObject(key)</c>，此后裸 key 即野句柄。</para>
	///   <para><b>与相邻成员的取舍</b><see cref="JlRegion(IntPtr)"/> 等价于本方法 copy=true；只是想给 <c>CopyKey()</c> 的产物找归宿时用 copy=false，省掉一次全量元组拷贝。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   JlRegion src = new JlRegion(10.0, 10.0, 30.0, 30.0);
	///   System.IntPtr key = src.CopyKey();
	///   JlRegion owner = new JlRegion(key, copy: false);    // 接管 key，之后不得再用裸 key
	///   JlRegion indep = new JlRegion(src.Key, copy: true); // 原生 id 568 深拷贝，与 src 无关
	///   owner.Dispose();
	///   indep.Dispose();
	///   src.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>copy=false 的实例 Dispose 之后调用方手里的同一个 IntPtr 已失效，需自行置空避免二次释放。</para>
	/// </remarks>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>从任意托管图标对象派生出区域实例：取其句柄做一次原生深拷贝，并强制校验它确实是 region 类。</summary>
	/// <param name="obj">来源托管对象，基类为 JlObject 的任意图标类型皆可传入。</param>
	/// <remarks>
	///   <para><b>功能说明</b>转基类 <c>JlObjectBase(JlObjectBase)</c>，即 <c>this(obj.key, copy: true)</c> 走原生 id 568 全量拷贝，再 <c>AssertObjectClass</c>（id 579）；方法体末尾 <c>GC.KeepAlive(this)</c>，基类构造里另有 <c>GC.KeepAlive(obj)</c>。</para>
	///   <para><b>约束或前提</b>传入未初始化对象（key 为 UNDEF）时拷贝被跳过，结果同样是未初始化实例；传入 <c>JlImage</c> 等非 region 句柄时抛 <c>JlException</c>，不会静默转成空区域。</para>
	///   <para><b>与相邻成员的取舍</b>手里已是 <c>JlRegion</c> 且要独立副本时 <c>Clone()</c> 更直接；本构造的价值在于只以 <c>JlObject</c> 身份流转的场合（集合元素、通用参数），拷完即与来源脱钩。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   JlObject boxed = new JlRegion(10.0, 10.0, 40.0, 60.0);
	///   JlRegion dup = new JlRegion(boxed);
	///   boxed.Dispose();
	///   dup.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>拷贝完成后 <paramref name="obj"/> 可先释放；<c>dup</c> 是独立新句柄，照常需要 Dispose。</para>
	/// </remarks>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>HIL 输出装载出口：新建空 JlRegion 并把原生过程的第 parIndex 个图标输出读进它，是本库所有返回新句柄的区域算子的统一实现。</summary>
	/// <param name="proc">当前原生过程句柄，即 PreCall 的返回值。</param>
	/// <param name="parIndex">图标输出序号，从 1 起计，与控制参数从 0 起计是两套独立编号。</param>
	/// <param name="err">已累积的原生返回码，失败时原样透传、不覆盖句柄。</param>
	/// <param name="obj">输出：装载后的区域实例，失败时仍是未初始化实例但非 null。</param>
	/// <returns>原生返回码，0 表示成功；失败码由调用方的 PostCall 统一抛出。</returns>
	/// <remarks>
	///   <para><b>功能说明</b>先 <c>new JlRegion(JlObjectBase.UNDEF)</c>（UNDEF 使基类拷贝分支跳过，零原生开销），再转实例 <c>Load</c>，后者经 <c>GetOutputObject(proc, parIndex, out key)</c> 直接接管原生输出句柄，不做二次拷贝。</para>
	///   <para><b>约束或前提</b>实例 <c>Load</c> 要求目标 key 必须为 UNDEF，否则会抛 JlException，因此本方法只能用于全新对象；<c>err</c> 已是失败码时不改写句柄。装载成功时若 <c>suppressedFinalization</c> 为真会重新注册终结器，对本方法刚 new 出的实例而言这一步是空操作。</para>
	///   <para><b>与相邻成员的取舍</b>本库所有返回新句柄的区域算子（<c>FillUpShape</c>、<c>SelectShape</c>、<c>Union2</c> 一族）都调本方法；<c>Gen*</c> 系列与角点构造器改走实例 <c>Load</c> 原地改写本实例，两种路径不要混用。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   System.IntPtr proc = JlNativeApi.PreCall(588);   // gen_rectangle1
	///   JlNativeApi.StoreD(proc, 0, 30.0);
	///   JlNativeApi.StoreD(proc, 1, 20.0);
	///   JlNativeApi.StoreD(proc, 2, 100.0);
	///   JlNativeApi.StoreD(proc, 3, 200.0);
	///   JlNativeApi.InitOCT(proc, 1);
	///   int err = JlNativeApi.CallProcedure(proc);
	///   err = JlRegion.LoadNew(proc, 1, err, out JlRegion roi);
	///   JlNativeApi.PostCall(proc, err);
	///   roi.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>输出的 <paramref name="obj"/> 所有权归调用方，无论装载成败都是一个非 null 的托管实例，都要 Dispose；未初始化实例的 Dispose 不触碰原生层。</para>
	/// </remarks>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>ISerializable 反序列化构造器：把 info 中名为 data 的字节块还原成本实例的区域句柄（deserialize_region，id 1572）。</summary>
	/// <param name="info">序列化载体，须含 GetObjectData 写入的 data 键（byte[]）。</param>
	/// <param name="context">流式上下文，本实现不使用。</param>
	/// <remarks>
	///   <para><b>功能说明</b>取 <c>(byte[])info.GetValue("data", typeof(byte[]))</c> 后转 <c>DeserializeRegion</c>：用 <c>JlSerializationBuffer</c> 包住字节、<c>PreCall(1572)</c> 以 <c>Store(proc, 0, buffer)</c> 送控制参数、<c>InitOCT(proc, 1)</c> 声明一个图标输出，再 <c>Load(proc, 1, err)</c> 原地填充刚构造的实例；配对方向是 <c>GetObjectData</c> 里的 <c>SerializeRegion()</c>（id 1573）。</para>
	///   <para><b>约束或前提</b>实例由本构造器新建、key 起点为 UNDEF，天然满足 <c>Load</c> 的前提；字节必须来自同一序列化族，缺 data 键或类型不符时 <c>GetValue</c> 先抛，到不了原生层。</para>
	///   <para><b>与相邻成员的取舍</b>序列化的是整条区域元组而非单个区域；不想引入 .NET 序列化框架时直接配 <c>SerializeRegion</c>/<c>DeserializeRegion</c>，落盘用 <c>Serialize(Stream)</c> 与静态 <c>Deserialize(Stream)</c>，内存内独立复制用 <c>Clone()</c>。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   JlRegion src = new JlRegion(10.0, 10.0, 30.0, 30.0);
	///   byte[] data = src.SerializeRegion();
	///   JlRegion back = new JlRegion();
	///   back.DeserializeRegion(data);   // 与本构造器走同一条 id 1572 路径
	///   back.Dispose();
	///   src.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>本构造器主要供序列化框架反射调用，已标 <c>EditorBrowsable(Never)</c>；还原出的句柄与常规实例一样需要 Dispose。</para>
	/// </remarks>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>线扫描接图（原生 id 467）：把本帧区域与上一帧残留区域按上下帧相接关系缝合，返回缝完的当前帧新句柄，未缝上部分经 out 另出新句柄；两个输入句柄均不被改写。</summary>
	/// <param name="prevRegions">上一轮迭代合并后的区域。</param>
	/// <param name="prevMergedRegions">上一轮迭代中未能与当前区域合并的区域。</param>
	/// <param name="imageHeight">Height of the line scan images. Default: 512</param>
	/// <param name="mergeBorder">当前图像中与上一图像相接的那一行。 Default: "top"</param>
	/// <param name="maxImagesRegion">单个区域所允许的最大图像数。 Default: 3</param>
	/// <returns>当前区域，在适用处已与旧区域合并。</returns>
	/// <remarks>
	...
	/// <summary>按固定 width×height 像素矩形瓦片网格切开区域、不看形状（原生 id 468）：一个连通块会切成多块瓦片，返回装全部瓦片的新句柄，输入句柄不被改写。</summary>
	/// <param name="width">Width of the individual rectangles.</param>
	/// <param name="height">Height of the individual rectangles.</param>
	/// <returns>分区后的区域。</returns>
	/// <remarks>
	...
	/// <summary>在区域纵向跨度小的位置（笔画间隙）沿水平方向下刀，把粘连区域拆成单字符（原生 id 469）：返回的新句柄里碎片已是分立元素，序号随切割位置变化；输入不被改写。</summary>
	/// <param name="distance">结果区域各部分的大致宽度。</param>
	/// <param name="percent">分割位置的最大百分比偏移。 Default: 20</param>
	/// <returns>分区后的区域。</returns>
	/// <remarks>
	...
	/// <summary>把区域元组逐元素栅格化为标签图（原生 id 470）：第 i 个元素的像素值为 i、背景为 0，画幅与像素类型由参数给定；返回新图像句柄，输入区域不被改写。</summary>
	/// <param name="type">结果图像的像素类型。 Default: "int2"</param>
	...
	/// <returns>尺寸为 Width * Height、包含转换后区域的结果图像。</returns>
	/// <remarks>
	...
	/// <summary>把区域栅格化为两级整数灰度图（原生 id 471）：区域像素取 foregroundGray、其余取 backgroundGray，画幅 width×height；返回新图像句柄，重叠区域不叠加也不区分。</summary>
	/// <param name="foregroundGray">区域的显示灰度值。 Default: 255</param>
	/// <param name="backgroundGray">背景的显示灰度值。 Default: 0</param>
	/// <param name="width">Width of the image to be generated. Default: 512</param>
	/// <param name="height">Height of the image to be generated. Default: 512</param>
	/// <returns>尺寸为 Width * Height、包含转换后区域的结果图像。</returns>
	/// <remarks>
	...
	/// <summary>一次产出两张图（原生 id 474）：返回值是每个像素到区域参考侧的距离图，out 给出最近点坐标图（"absolute" 时为绝对位置）；两图均为 width×height 新句柄，输入不被改写。</summary>
	/// <param name="closestPoints">包含最近点坐标的图像。</param>
	/// <param name="metric">Type of metric to be used for the closest-point transformation. Default: "city-block"</param>
	/// <param name="foreground">在区域内（'true'）还是区域外（'false'）计算像素距离。 Default: "true"</param>
	...
	/// <summary>把区域细化为 1 像素宽骨架（原生 id 476）：保持连通性与端点、分叉拓扑；返回新句柄，像素数远小于原区域，先取好的面积、矩等特征在骨架上全部作废须重算。</summary>
	/// <returns>所得的骨架。</returns>
	/// <remarks>
	...
	/// <summary>对区域施加单应矩阵的投影变换（原生 id 477）：允许近大远小，相机斜拍平面校正用它；结果是按像素栅格重采样的新句柄，输入不被改写。</summary>
	/// <param name="homMat2D">齐次射影变换矩阵。</param>
	/// <param name="interpolation">变换所用的插值方法。 Default: "bilinear"</param>
	/// <returns>输出区域。</returns>
	/// <remarks>
	...
	/// <summary>对区域施加任意 2D 仿射变换（原生 id 478）：返回新句柄、输入不被改写；栅格化后面积不保证恰等于原面积乘行列式绝对值，变换过的特征值一律重算。</summary>
	/// <param name="homMat2D">输入变换矩阵。</param>
	/// <param name="interpolate">变换是否使用插值进行？ Default: "nearest_neighbor"</param>
	/// <returns>变换后的输出区域。</returns>
	/// <remarks>
	...
	/// <summary>关于水平（row）或垂直（column）轴镜像区域（原生 id 479）：轴位于坐标 widthHeight/2 处——该参数是轴坐标的两倍而非图像宽高；返回新句柄，phi 等方向类量符号翻转。</summary>
	/// <param name="mode">Axis of symmetry. Default: "row"</param>
	/// <param name="widthHeight">对称轴坐标的两倍。 Default: 512</param>
	...
	/// <summary>区域坐标按 scaleWidth×scaleHeight 双轴缩放后重新栅格化（原生 id 480）：返回新句柄；面积一律随之改变，旧特征值作废，斜边台阶与细部丢失是像素集合重建的正常代价。</summary>
	/// <param name="scaleWidth">x 方向的缩放因子。 Default: 2.0</param>
	/// <param name="scaleHeight">y 方向的缩放因子。 Default: 2.0</param>
	...
	/// <summary>按整数像素向量平移区域（原生 id 481）：row 沿 y 向下为正、column 沿 x 向右为正；形状、面积、相对矩不变，只有质心、包框等绝对位置变；返回新句柄，移出画幅不报错。</summary>
	/// <param name="row">区域平移向量的行坐标。 Default: 30</param>
	/// <param name="column">区域平移向量的行坐标。 Default: 30</param>
	/// <returns>平移后的区域。</returns>
	/// <remarks>
	...
	/// <summary>从 1 像素宽骨架中挑出端点与分叉点两类点集（原生 id 482）：返回值是端点、out 是分叉点，各为新句柄；输入须先经 Skeleton 或 Golay 细化，毛刺多时端点数会爆。</summary>
	/// <param name="juncPoints">提取出的交点。</param>
	/// <returns>提取出的端点。</returns>
	/// <remarks>
	...
	/// <summary>生成区域（对）之间的缝隙区域（原生 id 484）：返回装分隔带的新句柄，间隙数量与配对方式有关，按序号取元素前先 CountObj 核对；输入不被改写。</summary>
	...
	///   JlRegion blobs = image.Threshold(100.0, 255.0).Connection();
	///   JlRegion filled = blobs.FillUpShape("area", 1.0, 200.0);
	///   int before = blobs.CountObj();
	///   int after = filled.CountObj();
	///   filled.Dispose();
	///   blobs.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>让区域逐圈外扩填缝或对重叠区按先到划界（原生 id 487）：本重载 iterations 以元组传入可逐对象给圈数，forbiddenArea 内不生长；返回新句柄，生长后面积、矩全部重算。</summary>
	...
	/// <param name="mode">膨胀模式。 Default: "image"</param>
	...
	/// <summary>整数圈数版的外扩填缝/重叠划界（与元组重载同一原生 id 487）：所有对象统一生长固定圈数，StoreI 直写、无元组固定与 UnpinTuple；返回新句柄，生长后特征须重算。</summary>
	/// <param name="forbiddenArea">Regions in which no expansion takes place.</param>
	/// <param name="iterations">Number of iterations. Default: "maximal"</param>
	/// <param name="mode">膨胀模式。 Default: "image"</param>
	/// <returns>Expanded or separated regions.</returns>
	...
	///   <code>
	///   JlImage image = new JlImage("cells.hobj");
	///   JlRegion seeds = image.Threshold(100.0, 255.0).Connection();
	///   JlRegion keepOut = new JlRegion(0.0, 0.0, 8.0, 512.0);
	///   using JlRegion grown = seeds.ExpandRegion(keepOut, 3, "image");
	///   int n = grown.CountObj();
	///   seeds.Dispose();
	///   keepOut.Dispose();
	///   image.Dispose();
	///   </code>
	...
	/// <summary>以每个对象自己的最小包框为基准裁边（原生 id 488）：top/bottom 裁行数、left/right 裁列数、逐对象生效，属破坏性裁剪；返回新句柄，裁后面积与包框改变须重算特征。</summary>
	...
	/// <summary>把区域裁进绝对坐标的整数矩形（原生 id 489）：等价于与该矩形求交，裁掉的像素不可恢复；返回新句柄，元组输入逐对象裁、元素顺序保持。</summary>
	/// <param name="row1">矩形左上角的行坐标。 Default: 0</param>
	/// <param name="column1">矩形左上角的列坐标。 Default: 0</param>
	/// <param name="row2">矩形右下角的行坐标。 Default: 256</param>
	/// <param name="column2">矩形右下角的列坐标。 Default: 256</param>
	/// <returns>裁剪后的区域。</returns>
	/// <remarks>
	...
	/// <summary>邻域计数滤波（原生 id 490）：像素在 width×height 掩膜内的前景像素数达到 number 才保留，取大值近似腐蚀、取小值近似去噪，转角会被圆化；返回新句柄，面积、矩全部改变。</summary>
	/// <param name="width">Width of the filter mask. Default: 15</param>
	/// <param name="height">Height of the filter mask. Default: 15</param>
	/// <param name="number">落在滤波模板内的最少点数。 Default: 70</param>
	/// <returns>所得的区域。</returns>
	/// <remarks>
	...
	/// <summary>对称差（原生 id 492）：只保留属于两侧之一的像素，A△B 不分方向，可一次拿到漏检+误检或量化形态学改动量；返回新句柄，region2 在返回前不得 Dispose。</summary>
	/// <param name="region2">输入区域 2。</param>
	/// <returns>所得的区域。</returns>
	/// <remarks>
	...
	/// <summary>取输入区域的补集并按连通性拆成背景块（原生 id 495，一步完成补集+拆分）：返回新句柄；背景块序号与目标块无对应关系，其面积普遍远大于目标块。</summary>
	/// <returns>背景的连通域。</returns>
	/// <remarks>
	...
	/// <summary>在 width×height 范围内随机改动恰好 distance 个像素的归属，生成与输入区域海明距离为 distance 的新区域（原生 id 496）：无种子参数、结果不可复现，供构造已知差异量的测试样本而非几何编辑。</summary>
	...
	/// <param name="distance">新旧区域之间的汉明距离。 Default: 1000</param>
	/// <returns>满足所需汉明距离的区域。</returns>
	/// <remarks>
	...
	/// <summary>按邻接判据删除无邻居的孤立前景点（原生 id 497，type 默认 "n_4"）：只删单点、不伤及 2 像素以上的细线；返回新句柄，去噪后像素数变化，面积类特征重算。</summary>
	...
	/// <summary>把每个对象代换为理想形状（原生 id 498，type 默认 "convex" 凸包）：代换后的区域仍可继续做布尔运算；返回新句柄、逐对象代换且元素顺序保持，面积与偏心率已变，代换前的特征不可再引用。</summary>
	...
	/// <summary>带灰度判据的区域生长（原生 id 499）：候选像素与区域边界的灰度/颜色差超过 threshold 即不越过，在同类灰度内填缝；本重载 iterations/threshold 以元组传入可逐对象给值，区域必须与 image 同一坐标系；返回新句柄，生长后特征重算。</summary>
	/// <param name="image">Image (possibly multi-channel) for gray value or color comparison.</param>
	/// <param name="forbiddenArea">Regions in which no expansion takes place.</param>
	/// <param name="iterations">Number of iterations. Default: "maximal"</param>
	/// <param name="mode">膨胀模式。 Default: "image"</param>
	/// <param name="threshold">区域边界处的灰度值或颜色与膨胀候选者之间的最大差值。 Default: 32</param>
	/// <returns>Expanded or separated regions.</returns>
	/// <remarks>
	...
	/// <summary>灰度生长的标量版（与元组重载同一原生 id 499）：iterations 走 StoreS（默认 "maximal" 长满即此写法）、threshold 走 StoreI 直写单一值，全体对象同一圈数、同一阈值时用；返回新句柄，生长后特征重算。</summary>
	/// <param name="image">Image (possibly multi-channel) for gray value or color comparison.</param>
	/// <param name="forbiddenArea">Regions in which no expansion takes place.</param>
	/// <param name="iterations">Number of iterations. Default: "maximal"</param>
	/// <param name="mode">膨胀模式。 Default: "image"</param>
	...
	///   JlRegion keepOut = new JlRegion(0.0, 0.0, 8.0, 512.0);
	///   using JlRegion grown = seeds.ExpandGray(image, keepOut, "maximal", "image", 40);
	///   int n = grown.CountObj();
	///   seeds.Dispose();
	///   keepOut.Dispose();
	///   image.Dispose();
	...
	/// <summary>与固定参考值比较的区域生长（原生 id 500）：候选像素与 refGray 之差超过 threshold 即停止，适合区域自身边界灰度不稳的场合；本重载 refGray/threshold 以元组传入；返回新句柄，image 与区域须同一坐标系，生长后特征重算。</summary>
	/// <param name="image">Image (possibly multi-channel) for gray value or color comparison.</param>
	/// <param name="forbiddenArea">Regions in which no expansion takes place.</param>
	/// <param name="iterations">Number of iterations. Default: "maximal"</param>
	/// <param name="mode">膨胀模式。 Default: "image"</param>
	/// <param name="refGray">用于比较的参考灰度值或颜色。 Default: 128</param>
	/// <param name="threshold">参考灰度值或颜色与膨胀候选者之间的最大差值。 Default: 32</param>
	/// <returns>Expanded or separated regions.</returns>
	/// <remarks>
	...
	/// <summary>固定参考灰度生长的标量版（与元组重载同一原生 id 500）：全体对象共用一个 refGray 与 threshold，StoreS/StoreI 直写、无元组固定与 UnpinTuple；返回新句柄，生长后特征重算。</summary>
	/// <param name="image">Image (possibly multi-channel) for gray value or color comparison.</param>
	/// <param name="forbiddenArea">Regions in which no expansion takes place.</param>
	/// <param name="iterations">Number of iterations. Default: "maximal"</param>
	/// <param name="mode">膨胀模式。 Default: "image"</param>
	...
	///   JlRegion seeds = image.Threshold(128.0, 255.0).Connection();
	///   JlRegion keepOut = new JlRegion(0.0, 0.0, 8.0, 512.0);
	///   using JlRegion grown = seeds.ExpandGrayRef(image, keepOut, "maximal", "image", 200, 40);
	///   int n = grown.CountObj();
	///   seeds.Dispose();
	///   keepOut.Dispose();
	///   image.Dispose();
	...
	/// <summary>把无分叉的 1 像素宽骨架段拟合为直线段（原生 id 501）：四个 out INTEGER 元组平行给出每段两端点坐标（像素），无返回值；maxDistance 是线点偏离两端连线的容差，调大并段、调小增段。</summary>
	/// <param name="maxDistance">线点相对于连接两端点的线段的最大距离。 Default: 3</param>
	/// <param name="beginRow">输出线段的起点行坐标。</param>
	/// <param name="beginCol">输出线段的起点列坐标。</param>
	/// <param name="endRow">输出线段的端点行坐标。</param>
	/// <param name="endCol">输出线段的端点列坐标。</param>
	/// <remarks>
	...
	/// <summary>把无分叉的 1 像素宽骨架段按"线点偏离两端连线不超过 maxDistance"拆成多段（原生 id 502）：结果仍是区域元组新句柄；段序号由拆分算法决定不可指定，要排序另走 SortRegion。</summary>
	/// <param name="maxDistance">线点相对于连接两端点的线段的最大距离。 Default: 3</param>
	/// <returns>分割后的直线。</returns>
	/// <remarks>
	...
	/// <summary>按行内游程长度删像素（原生 id 504）：短于 elimShorter 或长于 elimLonger 的游程整条删除，一维操作可保留竖向笔画；返回新句柄，连通块可能因个别游程被删而碎裂。</summary>
	/// <param name="elimShorter">较短的游程全部被消除。 Default: 3</param>
	/// <param name="elimLonger">较长的游程全部被消除。 Default: 1000</param>
	/// <returns>裁剪后的区域。</returns>
	/// <remarks>
	...
	/// <summary>对象元组级的整元素减法（原生 id 558）：从本句柄元素列表中删去与 objectsSub 内容一致的元素、不是删像素，剩下未被匹配走的对象；返回新句柄，剩余元素序号前移须重排后续按序号操作。</summary>
	...
	/// <summary>把区域按 grayval 涂画到 image 上并返回新图像（原生 id 561）：本重载 grayval 以元组传入可逐通道给值，type 取填充或只画边界；传入图像不被改写、保活到调用结束。</summary>
	/// <param name="image">Image in which the regions are to be painted.</param>
	/// <param name="grayval">区域所需的灰度值。 Default: 255.0</param>
	/// <param name="type">以填充或边界方式绘制区域。 Default: "fill"</param>
	...
	/// <summary>单一灰度值的涂画重载（与元组重载同一原生 id 561）：StoreD 直写、一个值适用所有通道，多通道图需逐通道不同值时不能用本重载；返回新图像句柄，输入图像不被改写。</summary>
	/// <param name="image">Image in which the regions are to be painted.</param>
	/// <param name="grayval">区域所需的灰度值。 Default: 255.0</param>
	/// <param name="type">以填充或边界方式绘制区域。 Default: "fill"</param>
	/// <returns>包含结果的图像。</returns>
	...
	///   JlImage image = new JlImage("coat.hobj");
	///   JlRegion defect = image.Threshold(200.0, 255.0);
	///   using JlImage painted = defect.PaintRegion(image, 0.0, "fill");
	///   defect.Dispose();
	///   image.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>把区域直接涂进 image 本体（原生 id 562）：无返回值、传入图像被原地改写，被覆盖处的原像素不可恢复；本重载 grayval 以元组传入按通道数配对。</summary>
	/// <param name="image">Image in which the regions are to be painted.</param>
	/// <param name="grayval">区域所需的灰度值。 Default: 255.0</param>
	/// <param name="type">以填充或边界方式绘制区域。 Default: "fill"</param>
	/// <remarks>
	...
	/// <summary>就地涂改的标量版（与元组重载同一原生 id 562）：StoreD 直写单一灰度值；image 被原地改写、无返回值，覆盖处原像素不可恢复。</summary>
	/// <param name="image">Image in which the regions are to be painted.</param>
	/// <param name="grayval">区域所需的灰度值。 Default: 255.0</param>
	/// <param name="type">以填充或边界方式绘制区域。 Default: "fill"</param>
	/// <remarks>
	...
	///   JlImage image = new JlImage("coat.hobj");
	///   JlRegion defect = image.Threshold(200.0, 255.0);
	///   defect.OverpaintRegion(image, 0.0, "fill");   // 无返回值，被改写的是 image 自己
	///   defect.Dispose();
	///   image.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>从当前对象列表按序号截连续区间整元素复制（原生 id 568）：自 index 起取 numObj 个、numObj=-1 取到末尾；返回新句柄，序号作用于当前列表，Connection/SortRegion/Union1 之后会错位。</summary>
	/// <param name="index">待复制对象的起始索引。 Default: 1</param>
	...
	/// <summary>把两个对象元组首尾相接（原生 id 569）：本句柄元素在前、objects2 在后，接的是整元素、像素重叠也仍是两块；返回新句柄，全部元素序号后移，两输入不被改写。</summary>
	...
	/// <summary>按序号向量从对象元组挑整元素（原生 id 572）：输出顺序按给定序号、同一序号可重复挑选即复制多份，返回新句柄；序号指"当前列表里的第几个"，上游拆分顺序不稳时先 SortRegion 定序。</summary>
	/// <param name="index">Indices of the objects to be selected. Default: 1</param>
	/// <returns>所选对象。</returns>
	/// <remarks>
	...
	/// <summary>按单一下标挑一个整元素（与元组重载同一原生 id 572）：StoreI 直写、无元组固定与 UnpinTuple，只取一个元素时用；返回新句柄，序号作用于当前列表顺序的错位风险同元组重载。</summary>
	/// <param name="index">Indices of the objects to be selected. Default: 1</param>
	/// <returns>所选对象。</returns>
	/// <remarks>
	...
	///   JlImage image = new JlImage("pellets.hobj");
	///   JlRegion blobs = image.Threshold(100.0, 255.0).Connection();
	///   using JlRegion third = blobs.SelectObj(3);
	///   int n = third.CountObj();
	///   blobs.Dispose();
	///   image.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>判断两个对象元组是否在容差内相同（原生 id 573）：返回经 LoadI 取出的标量 int 0/1，不是相似度分数；本重载 epsilon 以元组传入可逐属性给容差；不产生新句柄、不修改输入。</summary>
	/// <param name="objects2">测试对象。</param>
	/// <param name="epsilon">两个灰度值或坐标等之间的最大允许差值。 Default: 0.0</param>
	...
	/// <summary>容差比较的标量版（与元组重载同一原生 id 573）：StoreD 直写单一容差、全体属性共用，无元组固定与 UnpinTuple；返回标量 int 0/1，不产生新句柄、不修改输入。</summary>
	/// <param name="objects2">测试对象。</param>
	/// <param name="epsilon">两个灰度值或坐标等之间的最大允许差值。 Default: 0.0</param>
	/// <returns>布尔结果值。</returns>
	/// <remarks>
	...
	/// <summary>按位置逐对判断本句柄第 i 个区域是否被 region2 第 i 个区域包含（原生 id 574）：返回每元素 0/1 的 INTEGER 元组、不是标量布尔；两侧元素数不等或顺序不同即错配，先定序再用。</summary>
	/// <param name="region2">用于比较的区域。</param>
	...
	/// <summary>判断两侧区域是否逐像素一致（原生 id 575）：返回标量 int 0/1，判等要写 == 1；比的是像素集合不是句柄引用；经仿射或栅格化的结果几乎必然返回 0，验证重建应改容差比较。</summary>
	/// <param name="regions2">用于比较的区域。</param>
	/// <returns>布尔结果值。</returns>
	/// <remarks>
	...
	///   seed.GenRegionPoints(120, 340);        // 第 120 行、第 340 列的一个像素
	///   JlRegion blob = seed + 5.0;            // 以该点为种子膨胀出小块
	///   blob.Dispose();
	///   seed.Dispose();
	///   </code>
	/// </remarks>
	...
	///   <para><b>用法</b></para>
	///   <code>
	///   JlRegion bar = new JlRegion();
	///   bar.GenRegionRuns(120, 40, 260);       // 第 120 行上从第 40 列起的一段
	///   bar.Dispose();
	///   </code>
	/// </remarks>
	...
	///   roi.GenRectangle2(200.0, 300.0, 0.5236, 90.0, 20.0);   // 半轴 90、20，约 30° 的长条
	///   roi.Dispose();
	///   </code>
	/// </remarks>
	...
	///   <code>
	///   JlRegion roi = new JlRegion();
	///   roi.GenRectangle1(30.0, 20.0, 100.0, 200.0);
	///   JlRegion same = new JlRegion(30.0, 20.0, 100.0, 200.0);   // 与角点构造器等价
	///   roi.Dispose();
	///   same.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>在本实例上原地生成一团随机形状的连通区域（原生 id 589）：横向/纵向扩展不超过约 width/height 像素，先释放旧内容再装载、无返回值；签名无种子参数，形状不可复现，回归测试只断言规模与连通性等不变量。</summary>
	/// <param name="width">随机区域的最大水平扩展。 Default: 128</param>
	/// <param name="height">随机区域的最大垂直扩展。 Default: 128</param>
	/// <remarks>
	...
	///   <code>
	///   JlRegion hole = new JlRegion();
	///   hole.GenEllipse(150.0, 200.0, 0.7854, 40.0, 25.0);   // 45° 斜置的实心孔
	///   hole.Dispose();
	///   </code>
	/// </remarks>
	...
	///   <para><b>用法</b></para>
	///   <code>
	///   JlRegion fov = new JlRegion();
	///   fov.GenCircleSector(240.0, 320.0, 150.0, -0.5236, 0.5236);
	///   fov.Dispose();
	///   </code>
	/// </remarks>
	...
	///   disk.GenCircle(200.0, 200.0, 60.0);
	///   int area = disk.AreaCenter(out double cy, out double cx);   // 顺手核对面积与重心
	///   disk.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>生成棋盘格区域：每格宽高以像素计、范围以最大行列坐标界定，原地改写本实例（先 Dispose 再 Load）、无返回值，覆盖面积约为该范围的一半。</summary>
	/// <param name="widthRegion">区域内出现的最大 $x$ 值。 Default: 511</param>
	/// <param name="heightRegion">区域内出现的最大 $y$ 值。 Default: 511</param>
	...
	/// <summary>生成栅格线或点阵区域：行列步长以元组给出、可逐条给不等像素间距，图案限于给定像素宽高内，原地改写本实例（先 Dispose 再 Load）、无返回值。</summary>
	/// <param name="rowSteps">行方向的步宽或零。 Default: 10</param>
	/// <param name="columnSteps">列方向的步宽或零。 Default: 10</param>
	/// <param name="type">Type of created pattern. Default: "lines"</param>
	/// <param name="width">图样的最大宽度。 Default: 512</param>
	...
	/// <summary>等间距栅格线/点阵区域的标量重载：行列步长以整像素标量经 StoreI 直写，图案限于给定像素宽高内，原地改写本实例、无返回值。</summary>
	/// <param name="rowSteps">行方向的步宽或零。 Default: 10</param>
	/// <param name="columnSteps">列方向的步宽或零。 Default: 10</param>
	/// <param name="type">Type of created pattern. Default: "lines"</param>
	/// <param name="width">图样的最大宽度。 Default: 512</param>
	...
	///   <code>
	///   JlRegion grid = new JlRegion();
	///   grid.GenGridRegion(12, 12, "lines", 400, 300);
	///   int n = grid.CountObj();
	///   grid.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>批量生成随机圆/矩形/椭圆区域：尺寸区间按像素、转角区间按弧度给定并可逐对象用元组指定，中心散布在给定像素范围内，原地改写本实例、无返回值。</summary>
	/// <param name="type">Type of regions to be created. Default: "circle"</param>
	/// <param name="widthMin">对象特征的最小值，具体含义取决于类型与取值。 Default: 10.0</param>
	/// <param name="widthMax">对象特征的最大值，具体含义取决于类型与取值。 Default: 20.0</param>
	/// <param name="heightMin">对象特征的最小值，具体含义取决于类型与取值。 Default: 10.0</param>
	/// <param name="heightMax">对象特征的最大值，具体含义取决于类型与取值。 Default: 30.0</param>
	/// <param name="phiMin">区域的最小旋转角。 Default: -0.7854</param>
	/// <param name="phiMax">区域的最大旋转角。 Default: 0.7854</param>
	/// <param name="numRegions">Number of regions. Default: 100</param>
	/// <param name="width">中心点的最大水平扩展。 Default: 512</param>
	...
	/// <summary>随机区域族的标量重载：六个尺寸/转角区间以像素与弧度标量经 StoreD 直写、全体对象共用同一组区间，原地改写本实例、无返回值，且无种子参数故结果不可复现。</summary>
	/// <param name="type">Type of regions to be created. Default: "circle"</param>
	/// <param name="widthMin">对象特征的最小值，具体含义取决于类型与取值。 Default: 10.0</param>
	/// <param name="widthMax">对象特征的最大值，具体含义取决于类型与取值。 Default: 20.0</param>
	/// <param name="heightMin">对象特征的最小值，具体含义取决于类型与取值。 Default: 10.0</param>
	...
	///   <code>
	///   JlRegion dots = new JlRegion();
	///   dots.GenRandomRegions("circle", 12.0, 24.0, 12.0, 24.0, -0.5, 0.5, 40, 400, 300);
	///   int n = dots.CountObj();
	///   dots.Dispose();
	///   </code>
	/// </remarks>
	...
	///   <para><b>用法</b></para>
	///   <code>
	///   JlRegion target = new JlRegion(10.0, 10.0, 300.0, 400.0);
	///   JlRegion halfPlane = new JlRegion();
	///   halfPlane.GenRegionHline(0.0, 200.0);
	///   JlRegion clipped = target &amp; halfPlane;   // 法线式直线是无限长的，适合当裁剪界
	///   clipped.Dispose();
	///   halfPlane.Dispose();
	///   target.Dispose();
	///   </code>
	...
	///   <code>
	///   JlRegion rule = new JlRegion();
	///   rule.GenRegionLine(100, 50, 150, 250);   // 端点是整数像素索引，长度由两端点决定
	///   rule.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>把本实例原地改写为"已初始化但零像素"的空区域（原生侧无控制参数、无返回值），与未初始化实例不同，是并集累加器的合法起点。</summary>
	/// <remarks>
	...
	/// <summary>沿区域主轴量粗细：返回值给主轴方向的厚度剖面、out 给这些厚度值的直方图，两者都以 INTEGER 元组装载（像素档、非浮点），不改动输入句柄。</summary>
	...
	/// <summary>按像素容差把区域轮廓逼近成角点序列：本重载容差以元组钉传可逐对象给值，角点的先行后列像素坐标经两个 INTEGER out 元组给出，无 iconic 输出、不修改输入。</summary>
	/// <param name="tolerance">多边形与区域边界之间的最大距离。 Default: 5.0</param>
	/// <param name="rows">轮廓基点的行号。</param>
	/// <param name="columns">轮廓基点的列号。</param>
	/// <remarks>
	...
	/// <summary>多边形逼近的标量重载：容差为单一像素值经 StoreD 直写、所有对象共用，角点的先行后列像素坐标仍经两个 INTEGER out 元组给出，无 iconic 输出、不修改输入。</summary>
	/// <param name="tolerance">多边形与区域边界之间的最大距离。 Default: 5.0</param>
	/// <param name="rows">轮廓基点的行号。</param>
	/// <param name="columns">轮廓基点的列号。</param>
	/// <remarks>
	...
	///   <code>
	///   JlRegion part = new JlImage("plate.hobj").Threshold(100.0, 255.0);
	///   part.GetRegionPolygon(6.0, out JlTuple rows, out JlTuple cols);
	///   int n = rows.Length;
	///   rows.Dispose();
	///   cols.Dispose();
	///   part.Dispose();
	...
	/// <summary>把区域内每一个像素展开成先行后列的 INTEGER 坐标元组（导出量随面积线性增长），两个 out 调用处都必须写 out，无返回值、不改动输入句柄。</summary>
	/// <param name="rows">区域内像素的行号。</param>
	/// <param name="columns">区域内像素的列号。</param>
	/// <remarks>
	...
	/// <summary>导出区域边界像素的先行后列坐标序列（仍是像素集而非 XLD 亚像素轮廓），两个 out 均以 INTEGER 元组装载，无返回值、不改动输入句柄。</summary>
	/// <param name="rows">轮廓像素的行号。</param>
	/// <param name="columns">轮廓像素的列号。</param>
	/// <remarks>
	...
	/// <summary>导出区域的原生游程编码：行号与每条游程的起止列号以三个按序平行的 INTEGER 元组给出（量级为游程数而非像素数），三个 out 都得写 out，无返回值、不改输入。</summary>
	/// <param name="row">弦的行号。</param>
	/// <param name="columnBegin">弦的起点列号。</param>
	/// <param name="columnEnd">弦的端点列号。</param>
	/// <remarks>
	...
	/// <summary>以链码给轮廓：起点行列经 LoadI 取成两个标量 int 像素坐标、方向序列经 INTEGER 元组装载，三个 out 类型为 int/int/JlTuple 不可写错，无返回值、不改输入。</summary>
	/// <param name="row">Line of starting point.</param>
	/// <param name="column">Column of starting point.</param>
	/// <param name="chain">轮廓的方向码（自起点起）。</param>
	/// <remarks>
	...
	/// <summary>给出凸包的先行后列 INTEGER 像素点序列而非凸区域本身，经两个 out 输出（无返回值、不改输入）；要继续做布尔运算应改用 ShapeTrans 的 convex 变换。</summary>
	/// <param name="rows">轮廓像素的行号。</param>
	/// <param name="columns">轮廓像素的列号。</param>
	/// <remarks>
	...
	/// <summary>按像素位置关系重排对象元组：排序键以元组钉传、升降序与先行后列由另两个字符串决定，元素像素不变只换顺序，返回重排后的新句柄。</summary>
	...
	/// <summary>剪掉骨架上长度不足给定像素数的毛刺分支（长度经 StoreI 整数直写），只对 Skeleton/Thinning 产出的 1 像素宽输入有意义，返回新句柄、输入不变。</summary>
	...
	/// <summary>对结构元素元组里的每个元素各做一次"先开后闭"并留下最贴合的一版（像素级形态学复合，closing-after-opening），返回新句柄、输入区域与元素句柄都不被改写。</summary>
	/// <param name="structElements">多个结构元。</param>
	/// <returns>拟合得到的区域。</returns>
	/// <remarks>
	...
	/// <summary>生成标准结构元素并原地装入本实例（先 Dispose 再 Load、无返回值）：形状由 type 字符串决定，参考点以元素内的行列像素坐标给定，直接决定后续腐蚀/击中的对齐位置。</summary>
	/// <param name="type">Type of structuring element to generate. Default: "noise"</param>
	/// <param name="row">参考点的行坐标。 Default: 1</param>
	/// <param name="column">参考点的列坐标。 Default: 1</param>
	/// <remarks>
	...
	/// <summary>
	///   以整数像素点 (row,column) 为对称中心做点对称（原生 id 718）：每个前景像素 (r,c) 映到 (2·row−r, 2·column−c)，等价整体旋转 180°，返回新句柄、输入区域不变；参考点取整数故奇偶决定落格，质心移到对称点，绝对位置特征须重算。
	/// </summary>
	/// <param name="row">参考点的行坐标。 Default: 0</param>
	/// <param name="column">参考点的列坐标。 Default: 0</param>
	/// <returns>转置后的区域。</returns>
	/// <remarks>
	...
	/// <summary>
	///   顺序细化（原生 id 719）：按 Golay 字母表元素逐个"击中即删"，同一元素连打 iterations 轮（整数轮）且每轮的删除立即影响下一轮，故结果与元素顺序有关；本重载 iterations 以元组固定，可逐对象给不同轮数；返回新句柄、输入区域不变，细化改变全部特征值须重算。
	/// </summary>
	/// <param name="golayElement">Golay 字母表中的结构元。 Default: "l"</param>
	...
	/// <summary>序列细化的标量重载：同一 Golay 元素按序连打的迭代次数为整数标量、经 StoreI 直写，全体对象共用该次数，返回新句柄、输入区域不变。</summary>
	/// <param name="golayElement">Golay 字母表中的结构元。 Default: "l"</param>
	/// <param name="iterations">Number of iterations. For 'f', 'f2', 'h' and 'i' the only useful value is 1. Default: 20</param>
	/// <returns>Result of the thinning operator.</returns>
	/// <remarks>
	...
	///   <code>
	///   JlRegion part = new JlImage("maze.hobj").Threshold(128.0, 255.0);
	///   using JlRegion thin = part.ThinningSeq("e", 12);
	///   int n = thin.CountObj();
	///   part.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>
	///   并行 Golay 细化（原生 id 720）：把选定字母元素按整数 rotation 旋转后同时套到所有位置，命中处的前景像素在同一轮一起删掉，对称处理不偏向先扫到的位置、一轮只削一薄层；返回新句柄、输入区域不变，细化保拓扑不保尺寸，面积/矩/圆度全部重算，"h" 类元素反复调用会持续腐蚀成点。
	/// </summary>
	...
	/// <summary>
	///   通用细化（原生 id 721）：用两个自建元素定义击中即删——structElement1 压前景、structElement2 压背景补集，两者同时吻合的参考点处把前景移除，重复 iterations 轮（整数轮），(row,column) 决定元素如何对齐；返回新句柄、输入区域不变。两元素加参考点三者共同决定规则，参考点错位等于换了一个元素，轮数给大可能整条消失，先小步试。
	/// </summary>
	/// <param name="structElement1">用于前景的结构元。</param>
	/// <param name="structElement2">用于背景的结构元。</param>
	/// <param name="row">参考点的行坐标。 Default: 0</param>
	/// <param name="column">参考点的列坐标。 Default: 0</param>
	...
	/// <summary>
	///   顺序增厚（原生 id 722）：把选定 Golay 元素按序连打 iterations 轮（整数轮），每轮在所有命中空位把像素补进区域，是细化族的对偶——只在满足击中条件的局部补点、不像膨胀那样无差别外扩，故保拓扑，用于修补细化过度的断裂；返回新句柄、输入区域不变，增厚与膨胀一样改变全部特征值须重算。
	/// </summary>
	/// <param name="golayElement">Golay 字母表中的结构元。 Default: "h"</param>
	/// <param name="iterations">Number of iterations. Default: 1</param>
	...
	///   各字母的合法取值集合 [待实测]。需要旋转元素请改用 <c>ThickeningGolay</c>（id 723，带 rotation）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	...
	/// <summary>
	///   并行 Golay 增厚（原生 id 723）：把选定元素按整数 rotation 旋转后同时判所有位置，命中的空位在一轮里一起补进区域，与 <c>ThinningGolay</c>（id 720）严格对偶；一轮只加一薄层、对称处理，恢复细化过度的断口而不像膨胀那样把骨架整体加粗；返回新句柄、输入区域不变，增厚后面积/矩等特征值全部重算。
	/// </summary>
	/// <param name="golayElement">Golay 字母表中的结构元。 Default: "h"</param>
	/// <param name="rotation">Rotation of the Golay element. Depending on the element, not all rotations are valid. Default: 0</param>
	/// <returns>Result of the thickening operator.</returns>
	...
	/// <summary>
	///   通用增厚（原生 id 724）：两个自建掩膜加参考点定义击中即补——structElement1 盖前景、structElement2 盖背景补集，两者同时吻合处把像素补进区域，重复 iterations 轮（整数轮），是 <c>Thinning</c>（id 721）的对偶；返回新句柄、输入区域不变。两元素的相对位置与参考点共同决定规则：配套错了会整体偏移，或把补缝变成糊死，结果特征值全部重算。
	/// </summary>
	/// <param name="structElement1">用于前景的结构元。</param>
	/// <param name="structElement2">用于背景的结构元。</param>
	/// <param name="row">参考点的行坐标。 Default: 16</param>
	/// <param name="column">参考点的列坐标。 Default: 16</param>
	/// <param name="iterations">Number of iterations. Default: 1</param>
	/// <returns>Result of the thickening operator.</returns>
	/// <remarks>
	...
	/// <summary>
	///   序列 Golay 击中即中检测（原生 id 725，按序处理）：交出"元素前景部分落在区域内、其背景部分落在区域外"的那些参考点组成的点集区域；坐标语义是参考点像素位置而非元素覆盖范围，与原图叠加时差一个参考点偏移；返回新句柄、原区域不动——只定位角点/端点/交叉点，要删前景才用 <c>ThinningSeq</c>（id 719），一轮扫全部位置并带旋转角用 <c>HitOrMissGolay</c>（id 726）。
	/// </summary>
	/// <param name="golayElement">Golay 字母表中的结构元。 Default: "h"</param>
	...
	/// <summary>
	///   并行 Golay 击中即中（原生 id 726）：把选定元素按整数 rotation 旋转后一轮扫过所有位置，一次交出全部命中参考点构成的点集区域（新句柄），原区域不变、只报告位置不做删加；输出坐标的语义是参考点位置，找角点位置用它，完全自定义两掩膜判据改用 <c>HitOrMiss</c>（id 727）。
	/// </summary>
	/// <param name="golayElement">Golay 字母表中的结构元。 Default: "h"</param>
	/// <param name="rotation">Rotation of the Golay element. Depending on the element, not all rotations are valid. Default: 0</param>
	/// <returns>Result of the hit-or-miss operation.</returns>
	...
	/// <summary>双掩膜命中判断：输出前景掩膜放得进、背景掩膜落在补集里的位置集合，返回新句柄。</summary>
	/// <param name="structElement1">用于输入区域的腐蚀模板。</param>
	/// <param name="structElement2">用于输入区域补集的腐蚀模板。</param>
	/// <param name="row">参考点的行坐标。 Default: 16</param>
	/// <param name="column">参考点的列坐标。 Default: 16</param>
	/// <returns>Result of the hit-or-miss operation.</returns>
	/// <remarks>
	...
	/// <summary>
	///   取 Golay 字母表的一个结构元（原生 id 728）：调用把 this 就地换成前景模板 SE（旧内容当场释放，不再可用），返回的新句柄才是配套的背景模板 SEB，两者成对喂给 <c>HitOrMiss</c>/<c>ThinningGolay</c> 一族自制命中或细化判据；row/column 是模板参考点的整数像素坐标，rotation 只对方位对称的元素有效；要整套细化流程直接用 <c>ThinningSeq</c>/<c>ThinningGolay</c>，不必自取元素。
	/// </summary>
	/// <param name="golayElement">Name of the structuring element. Default: "l"</param>
	/// <param name="rotation">Rotation of the Golay element. Depending on the element, not all rotations are valid. Default: 0</param>
	/// <param name="row">参考点的行坐标。 Default: 16</param>
	/// <param name="column">参考点的列坐标。 Default: 16</param>
	/// <returns>用于背景的结构元。</returns>
	/// <remarks>
	...
	/// <summary>Golay 顺序细化（元组迭代数重载）：先用元素 'l' 至多迭代 iterations1 次、再用 'e' 至多 iterations2 次，产出 1 像素宽骨架状区域，返回新句柄。</summary>
	...
	/// <summary>Golay 顺序细化（整数迭代数重载）：'l' 至多迭代 iterations1 次、'e' 至多 iterations2 次，StoreI 直写无钉固定开销，返回新句柄。</summary>
	/// <param name="iterations1">Number of iterations for the sequential thinning with the element 'l' of the Golay alphabet. Default: 100</param>
	/// <param name="iterations2">Number of iterations for the sequential thinning with the element 'e' of the Golay alphabet. Default: 1</param>
	/// <returns>Result of the skiz operator.</returns>
	/// <remarks>
	...
	/// <summary>形态学骨架：逐区域计算保持连通性的 1 像素宽拓扑骨架，输入不变，返回新句柄。</summary>
	/// <returns>所得的形态学骨架。</returns>
	/// <remarks>
	...
	/// <summary>礼帽与黑礼帽之并：一次取全"结构元装不下"的凸起与凹陷细节，输入不变，返回新句柄。</summary>
	/// <param name="structElement">结构元（与位置无关）。</param>
	...
	/// <summary>黑礼帽：闭运算结果减去原区域，抓结构元能盖住而原区域缺着的外部凹陷与缝隙，返回新句柄。</summary>
	/// <param name="structElement">结构元（与位置无关）。</param>
	...
	/// <summary>礼帽：原区域减去开运算结果，剩下放不下结构元的凸起、毛刺与端点，返回新句柄。</summary>
	/// <param name="structElement">结构元（与位置无关）。</param>
	...
	/// <summary>带参考点的 Minkowski 差腐蚀：把结构元参考点平移到 (row, column) 后迭代腐蚀，返回新句柄。</summary>
	/// <param name="structElement">结构元。</param>
	/// <param name="row">参考点的行坐标。 Default: 0</param>
	/// <param name="column">参考点的列坐标。 Default: 0</param>
	/// <param name="iterations">Number of iterations. Default: 1</param>
	/// <returns>腐蚀后的区域。</returns>
	/// <remarks>
	...
	/// <summary>Minkowski 差腐蚀：按结构元自带参考点迭代 iterations 次腐蚀，输入不变，返回新句柄。</summary>
	/// <param name="structElement">结构元。</param>
	/// <param name="iterations">Number of iterations. Default: 1</param>
	/// <returns>腐蚀后的区域。</returns>
	/// <remarks>
	...
	/// <summary>带参考点的 Minkowski 和膨胀：把结构元参考点平移到 (row, column) 后迭代膨胀，返回新句柄。</summary>
	/// <param name="structElement">结构元。</param>
	/// <param name="row">参考点的行坐标。</param>
	/// <param name="column">参考点的列坐标。</param>
	/// <param name="iterations">Number of iterations. Default: 1</param>
	/// <returns>膨胀后的区域。</returns>
	/// <remarks>
	...
	/// <summary>Minkowski 和膨胀：按结构元自带参考点迭代 iterations 次膨胀，输入不变，返回新句柄。</summary>
	/// <param name="structElement">结构元。</param>
	/// <param name="iterations">Number of iterations. Default: 1</param>
	/// <returns>膨胀后的区域。</returns>
	/// <remarks>
	...
	///   <code>
	///   JlImage image = new JlImage("pcb.hobj");
	///   JlRegion blob = image.Threshold(120.0, 255.0);
	///   blob.DistanceSr(10.0, 10.0, 10.0, 300.0, out double distMin, out double distMax);
	///   blob.Dispose();
	///   </code>
	/// </remarks>
	...
	///   <code>
	///   JlImage image = new JlImage("pcb.hobj");
	///   JlRegion blobs = image.Threshold(120.0, 255.0).Connection();
	///   blobs.DistanceLr(new JlTuple(10.0), new JlTuple(10.0), new JlTuple(10.0), new JlTuple(300.0),
	///       out JlTuple distMin, out JlTuple distMax);   // 纯数值元组，无需显式释放
	///   blobs.Dispose();
	///   </code>
	/// </remarks>
	...
	///   JlImage image = new JlImage("pcb.hobj");
	///   JlRegion blob = image.Threshold(120.0, 255.0);
	///   blob.DistanceLr(10.0, 10.0, 10.0, 300.0, out double distMin, out double distMax);
	///   blob.Dispose();
	///   </code>
	/// </remarks>
	...
	///   JlImage image = new JlImage("pcb.hobj");
	///   JlRegion blobs = image.Threshold(120.0, 255.0).Connection();
	///   blobs.DistancePr(new JlTuple(120.0), new JlTuple(240.0),
	///       out JlTuple distMin, out JlTuple distMax);
	///   blobs.Dispose();
	///   </code>
	/// </remarks>
	...
	///   JlImage image = new JlImage("pcb.hobj");
	///   JlRegion blob = image.Threshold(120.0, 255.0);
	///   blob.DistancePr(120.0, 240.0, out double distMin, out double distMax);
	///   blob.Dispose();
	///   </code>
	/// </remarks>
	...
	///   JlImage image = new JlImage("parts.hobj");
	///   JlRegion part = image.Threshold(100.0, 255.0);
	///   part.SmallestRectangle2(out double cy, out double cx, out double phi,
	///       out double length1, out double length2);
	///   JlRegion roi = new JlRegion();
	///   roi.GenRectangle2(cy, cx, phi, length1, length2);   // 五个出参与生成端同序同义
	///   roi.Dispose();
	///   part.Dispose();
	///   </code>
	/// </remarks>
	...
	///   JlImage image = new JlImage("labels.hobj");
	///   JlRegion blob = image.Threshold(90.0, 255.0);
	///   blob.SmallestRectangle1(out int row1, out int column1, out int row2, out int column2);
	///   JlRegion crop = new JlRegion();
	///   crop.GenRectangle1((double)row1, (double)column1, (double)row2, (double)column2);   // INTEGER 角点显式转 double 形参（免与 JlTuple 重载二义）
	///   crop.Dispose();
	///   blob.Dispose();
	///   </code>
	/// </remarks>
	...
	///   <code>
	///   JlImage image = new JlImage("coins.hobj");
	///   JlRegion coin = image.Threshold(120.0, 255.0);
	///   coin.SmallestCircle(out double cy, out double cx, out double radius);
	///   JlRegion mask = new JlRegion();
	///   mask.GenCircle(cy, cx, radius);   // 外接圆半径按最远像素定径，比内切圆系统性偏大
	///   mask.Dispose();
	///   coin.Dispose();
	///   </code>
	/// </remarks>
	...
	///   <code>
	///   JlImage image = new JlImage("wafer.hobj");
	///   JlRegion pads = image.Threshold(120.0, 255.0).Connection();
	///   JlRegion site = new JlRegion(100.0, 100.0, 160.0, 160.0);
	///   JlRegion covered = pads.SelectShapeProto(site, "covers", 50.0, 100.0);
	///   covered.Dispose();
	///   site.Dispose();
	///   pads.Dispose();
	///   </code>
	...
	///   JlImage image = new JlImage("coins.hobj");
	///   JlRegion blobs = image.Threshold(120.0, 255.0).Connection();
	///   JlRegion big = blobs.SelectShape("area", "and", 1000.0, 99999.0);
	///   big.Dispose();
	///   blobs.Dispose();
	///   </code>
	/// </remarks>
	...
	///   JlRegion pads = image.Threshold(120.0, 255.0).Connection();
	///   JlRegion keepOut = image.Threshold(200.0, 255.0);
	///   double distMin = pads.DistanceRrMin(keepOut, out int row1, out int column1,
	///       out int row2, out int column2);   // 距离是 DOUBLE，四个近点坐标按 INTEGER 装载
	///   keepOut.Dispose();
	///   pads.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>逐像素比对两批区域，给出每对的差异像素数与相似度（不做平移对齐）。</summary>
	/// <param name="regions2">与本句柄按序号一一配对的第二批区域。</param>
	/// <param name="similarity">各对的相似度（DOUBLE 元组）。</param>
	/// <returns>各对的汉明距离（INTEGER 元组）：两区域对称差的像素数。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>把 <c>this</c> 的第 n 个区域与 <c>regions2</c> 的第 n 个区域直接叠在一起数不同的像素，
	...
	///   顺序不稳定的算子，两侧顺序一变就静默算错。两侧区域个数不等时的行为 [待实测]。</para>
	...
	///   实现把 <c>this</c> 放 iconic 槽 1、<c>regions2</c> 放槽 2。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlRegion a = new JlRegion(10.0, 10.0, 30.0, 30.0);
	///   JlRegion b = new JlRegion(12.0, 10.0, 32.0, 30.0);
	///   JlTuple distance = a.HammingDistance(b, out JlTuple similarity);
	///   a.Dispose();
	///   b.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>两条输出都是纯数值 <c>JlTuple</c>，可不显式释放；两输入句柄
	///   <c>GC.KeepAlive</c> 到原生调用结束，示例之后释放即可。</para>
	/// </remarks>
	...
	/// <summary>标量重载：只取第 1 对区域的汉明距离与相似度（同 id 1636）。</summary>
	/// <param name="regions2">与本句柄配对的第二批区域。</param>
	/// <param name="similarity">第 1 对的相似度（<c>double</c>）。</param>
	/// <returns>第 1 对的汉明距离（<c>int</c>，对称差像素数）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>与元组版跑同一个原生算子，语义是不做对齐的逐像素差异；
	///   差别只在装载方式：<c>LoadI</c>/<c>LoadD</c> 各取输出的第 1 个值。</para>
	///   <para><b>约束或前提</b>两侧都按序号配对，本重载等于假定"我只有一对"。
	///   句柄里含多个区域时，第 2 个及以后的结果被静默丢弃——这是本重载唯一的坑。</para>
	///   <para><b>与相邻算子的取舍</b>成批比对必须用 <c>HammingDistance(JlRegion, out JlTuple)</c>；
	///   允许平移错位则两个重载都不合适，改用 <c>HammingDistanceNorm</c>（id 1635）先对齐。</para>
	///   <para><b>参数取向</b>返回 1 个标量、<c>out</c> 1 个标量，不建 <c>JlTuple</c>，无额外分配。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlRegion tmpl = new JlRegion(10.0, 10.0, 30.0, 30.0);
	///   JlRegion cur = new JlRegion(11.0, 10.0, 31.0, 30.0);
	///   int distance = cur.HammingDistance(tmpl, out double similarity);
	///   cur.Dispose();
	///   tmpl.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>不返回句柄；两输入句柄 <c>GC.KeepAlive</c> 到调用结束。
	///   单像素平移就会给出与形状变化同量级的距离，判阈值前先确认两者已对齐。</para>
	/// </remarks>
	...
	/// <summary>由等效椭圆参数派生的三条形状量：各向异性度（返回值）、块度、结构因子。</summary>
	/// <param name="bulkiness">形状量（DOUBLE 元组，逐区域）。</param>
	/// <param name="structureFactor">形状量（DOUBLE 元组，逐区域）。</param>
	/// <returns>各向异性度量（DOUBLE 元组）：正圆时为 1.0。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>三条输出都来自二阶矩算出的等效椭圆半轴（见 id 1640 的
	///   <c>EllipticAxis</c>），因此只反映"惯性上像不像圆"，不看轮廓是否毛糙；
	///   返回值对正圆等于 1.0，越扁越大。</para>
	///   <para><b>约束或前提</b>逐区域计算，句柄里有多个连通域时三条元组与区域序号对齐。
	///   <c>Connection</c> 之后顺序不稳会整条错位；膨胀/腐蚀改了二阶矩，必须重取本量，
	///   沿用旧值等于用形态学前的形状判形。单像素或退化区域给出的值 [待实测]。</para>
	///   <para><b>与相邻算子的取舍</b>要长短轴和倾角本身用 <c>EllipticAxis</c>；要"边是否圆"
	///   用 <c>Roundness</c>（轮廓距离统计）；要凸凹程度用 <c>Convexity</c>。三者对同一片噪声
	///   的反应完全不同，别互相顶替。</para>
	///   <para><b>参数取向</b>返回 1 条、<c>out</c> 2 条，原生出参序 [0]=各向异性、[1]=bulkiness、
	///   [2]=structureFactor，与形参顺序一致（id 1637）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("bottle.hobj");
	///   JlRegion caps = image.Threshold(80.0, 255.0).Connection();
	///   JlTuple anisometry = caps.Eccentricity(out JlTuple bulkiness, out JlTuple structureFactor);
	///   caps.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>三条纯数值元组无需释放；本量不含句柄输出。坑在于细长但规则的
	///   物体（针、划痕）与短粗但边缘破碎的物体可能给出接近的块度，判形时至少并一条矩无关的量。</para>
	/// </remarks>
	...
	/// <summary>等效椭圆形状量（标量版）：只取句柄第 1 个区域的三条值。</summary>
	/// <param name="bulkiness">第 1 个区域的块度。</param>
	/// <param name="structureFactor">第 1 个区域的结构因子。</param>
	/// <returns>第 1 个区域的各向异性度（正圆为 1.0）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>与元组版同一原生算子（id 1637），改为三次 <c>LoadD</c> 直取标量，
	...
	///   <c>Connection</c> 之后未排序时第 1 个未必是目标域 [待实测其排序依据]。</para>
	///   <para><b>与相邻算子的取舍</b>单域快速判形用它，省掉元组开销；成批筛形走元组版配
	///   <c>SelectShape</c>。</para>
	///   <para><b>参数取向</b>返回 1 标量 + <c>out</c> 2 标量，出参序仍是各向异性、bulkiness、
	///   structureFactor。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlRegion seal = new JlRegion(20.0, 20.0, 60.0, 60.0);
	///   double anisometry = seal.Eccentricity(out double bulkiness, out double structureFactor);
	///   seal.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>无句柄、无元组分配。坑同上：多区域时结果只对应第 1 个，
	///   拿它当"整体形状指标"会误读。</para>
	/// </remarks>
	...
	/// <summary>逐区域给出欧拉数：连通域个数减去孔的个数（INTEGER 元组）。</summary>
	/// <returns>各区域的欧拉数（<c>int</c> 元组）：实心块为 1，每多一个孔减 1。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>拓扑量，只看"几块、几个洞"，与面积、周长、朝向无关；
	///   带孔垫圈为 0，两孔为 -1。句柄里若装了多个区域，逐区域各给一个值。</para>
	///   <para><b>约束或前提</b>像素连通性判定（对角接触算不算连通）直接决定结果，本库口径 [待实测]；
	///   因此 <c>Connection</c> 拆分的粒度与这里的计数用的是同一套邻域约定。
	///   <c>FillUp</c> 会把孔填掉使欧拉数升到 1；一次膨胀也可能把缝合成孔、把两处接触并成一块，
	///   形态学之后必须重算，别拿预处理前的计数当结论。</para>
	///   <para><b>与相邻算子的取舍</b>想知道"几块/几孔"各是多少用 <c>ConnectAndHoles</c>（id 1645），
	///   它把两个数分开给；只要"是否有孔"这类判定时两者的差值信息更省。
	///   统计整套区域的块数用 <c>CountObj()</c>，与本量不是一回事。</para>
	///   <para><b>参数取向</b>无入参、返回 1 条 INTEGER 元组（id 1638）；本类没有标量重载，
	///   想把值走 <c>out</c> 取可用静态 <c>JlOperatorSet.EulerNumber(JlObject, out JlTuple)</c>。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("washer.hobj");
	///   JlRegion parts = image.Threshold(60.0, 255.0).Connection();
	///   JlTuple euler = parts.EulerNumber();
	///   int n = parts.CountObj();
	///   parts.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>只读量测，不产生句柄；<c>parts</c> 是 <c>Connection</c> 的新句柄要释放。
	///   坑：区域边缘的单像素毛刺不改变欧拉数，但把背景夹成"假孔"时会给出 -1，
	///   判孔前先 <c>FillUp</c> 对比一次。</para>
	/// </remarks>
	...
	/// <summary>区域的朝向：二阶矩主轴与水平方向的夹角，弧度制（DOUBLE 元组）。</summary>
	/// <returns>各区域的朝向角（<c>double</c> 元组，弧度）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>取惯性主轴的方向，等价于 <c>EllipticAxis</c> 的 <c>phi</c>（id 1640 的第 3 个输出）；
	///   只有一条边方向明显占优时才有意义。图像的 row 轴向下，因此该角的正负方向与数学习惯相反，
	///   与 <c>phi</c> 的具体符号换算关系 [待实测]，写姿态判据前先实测一例确认。</para>
	...
	///   <c>Eccentricity</c> 再信本量。</para>
	///   <para><b>与相邻算子的取舍</b>要长短轴一起拿就用 <c>EllipticAxis</c>；要"最小外接矩形"的边方向
	///   （对 L 形比主轴更稳）用 <c>SmallestRectangle2</c>；本量对离群毛刺敏感，矩形类的不敏感。</para>
	///   <para><b>参数取向</b>无入参、返回 1 条 DOUBLE 元组（原生 id 1639），本类无标量重载。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("screws.hobj");
	///   JlRegion screws = image.Threshold(90.0, 255.0).Connection();
	///   JlTuple phi = screws.OrientationRegion();
	///   screws.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>纯数值元组，无需释放。<c>Connection</c> 的新句柄要释放。
	///   坑：形态学之后角度会变（腐蚀掉一侧毛刺就可能把主轴拧过去），角度阈值判据要在最终区域上一次算完。</para>
	/// </remarks>
	...
	/// <summary>等效椭圆三参数：主轴半径（返回值）、次轴半径、主轴与 x 轴夹角（弧度）。</summary>
	/// <param name="rb">次轴半径（DOUBLE 元组，与主轴同一面积归一口径）。</param>
	/// <param name="phi">主轴与 x 轴夹角，弧度（DOUBLE 元组）。</param>
	/// <returns>主轴半径（DOUBLE 元组）。</returns>
	/// <remarks>
	...
	///   <c>OrientationRegion</c> 同源但符号口径 [待实测]；<c>ra/rb</c> 即
	///   <c>Eccentricity</c>（id 1637）的第一条输出，两者不必都算一遍。
	///   退化区域（空、单像素）的半径取值 [待实测]。</para>
	...
	///   <para><b>参数取向</b>返回 1 条、<c>out</c> 2 条；原生出参槽 [0]=ra、[1]=rb、[2]=phi，
	///   与形参顺序一致（id 1640）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("bolts.hobj");
	///   JlRegion bolts = image.Threshold(100.0, 255.0).Connection();
	///   JlTuple ra = bolts.EllipticAxis(out JlTuple rb, out JlTuple phi);
	///   bolts.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>三条数值元组无需释放。<c>Connection</c> 的新句柄要释放。
	///   坑：形态学（尤其膨胀）会同时改半径与比值，跨步骤比较 <c>ra</c> 必须取自同一阶段的区域。</para>
	/// </remarks>
	...
	/// <summary>等效椭圆三参数（标量版）：只取句柄第 1 个区域的 ra/rb/phi。</summary>
	/// <param name="rb">输出：次轴半径。</param>
	/// <param name="phi">输出：主轴与 x 轴夹角，弧度。</param>
	/// <returns>主轴半径（<c>double</c>）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>与元组版同一原生算子（id 1640），三次 <c>LoadD</c> 直取标量，
	///   不建 <c>JlTuple</c>，出参序仍是 ra、rb、phi。</para>
	///   <para><b>约束或前提</b>只对"单区域句柄"有意义：句柄内含多个区域时只拿到第 1 个，
	///   其余静默丢弃，且第 1 个是谁取决于上游顺序（<c>Connection</c> 不保证按位置出）。
	///   多域请走元组版。</para>
	///   <para><b>与相邻算子的取舍</b>只要倾角不要半径时用 <c>OrientationRegion</c>；
	///   半径要换成实际外框尺寸就用 <c>SmallestRectangle2</c> 的半边长，两者对非凸区域差得很远。</para>
	///   <para><b>参数取向</b>返回 1 标量 + <c>out</c> 2 标量。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlRegion bar = new JlRegion(50.0, 20.0, 62.0, 180.0);
	///   double ra = bar.EllipticAxis(out double rb, out double phi);
	///   bar.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>无句柄、无元组分配。<c>phi</c> 的符号受图像行轴向下影响，
	///   与几何直觉相反时先确认口径 [待实测]。</para>
	/// </remarks>
	...
	/// <summary>按方位关系在两批区域间配对：返回命中方的序号，out 给出对侧配对序号。</summary>
	/// <param name="regions2">被考察的第二批区域。</param>
	/// <param name="direction">方位关系，单值字符串。Default: "left"</param>
	/// <param name="regionIndex2">与返回值逐位配对的 <c>regions2</c> 侧序号（INTEGER 元组）。</param>
	/// <returns>本句柄中命中的区域序号（INTEGER 元组，1 基）。</returns>
	/// <remarks>
	...
	///   <para><b>约束或前提</b>方位判定用的参考点（质心还是包围盒边缘）以及边界相切算不算命中 [待实测]；
	///   <c>direction</c> 合法取值集合本层未枚举 [待实测]。两侧必须是同一坐标系同一尺度的区域，
	...
	///   要一次给多个方位词，用静态 <c>JlOperatorSet.SelectRegionSpatial(JlObject, JlObject, JlTuple, out JlTuple, out JlTuple)</c>，
	///   其 <c>direction</c> 是 <c>JlTuple</c>，且两串序号都走 <c>out</c>。</para>
	///   <para><b>参数取向</b>返回 1 条 + <c>out</c> 1 条；<c>direction</c> 以 <c>StoreS</c> 直写控制参数 0，
	///   不钉固定元组；<c>this</c> 进 iconic 槽 1、<c>regions2</c> 进槽 2（原生 id 1641）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("circuit.hobj");
	///   JlRegion pads = image.Threshold(120.0, 255.0).Connection();
	///   JlRegion holes = image.Threshold(0.0, 60.0).Connection();
	///   JlTuple padIdx = pads.SelectRegionSpatial(holes, "left", out JlTuple holeIdx);
	///   JlRegion matched = holes.SelectObj(holeIdx);
	///   pads.Dispose();
	///   holes.Dispose();
	///   matched.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>本算子不分配句柄，但示例里 <c>SelectObj</c> 与 <c>Connection</c> 都是新句柄，需各自释放。
	///   坑：一侧无命中时序号元组为空，拿空元组去 <c>SelectObj</c> 的行为 [待实测]，先判长度再用。</para>
	/// </remarks>
	...
	/// <summary>按面积占比门限在两批区域间找方位配对，并回赠双方的竖直/水平方位字符串。</summary>
	/// <param name="regions2">被考察的第二批区域。</param>
	/// <param name="percent">对照区域需有百分之多少落在指定侧才算成立（整型门限）。Default: 50</param>
	/// <param name="regionIndex2">命中的 <c>regions2</c> 侧序号（INTEGER 元组，1 基）。</param>
	/// <param name="relation1">逐位给出 <c>regionIndex2[n]</c> 相对 <c>regionIndex1[n]</c> 的竖直方位（字符串元组）。</param>
	/// <param name="relation2">同上，水平方位（字符串元组）。</param>
	/// <returns>命中的本句柄侧序号（INTEGER 元组，1 基）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>四条输出逐位对齐成"第 n 对"：返回值与 <c>regionIndex2</c> 是序号，
	///   <c>relation1</c> 是竖直方向、<c>relation2</c> 是水平方向，合起来定位这对区域谁在谁的哪一侧。</para>
	///   <para><b>约束或前提</b><c>percent</c> 是面积占比门限（整型写死，<c>StoreI</c> 直写控制参数 0），
	///   它决定"半压着算不算"：调低会把只是擦边的域也配进来。区域面积本身要可信，
	///   形态学之后面积变了就得重算，不能沿用旧的门限判断。</para>
	///   <para><b>与相邻算子的取舍</b>只要方位、不要面积占比且不想处理字符串时用
	///   <c>SelectRegionSpatial</c>（id 1641）更省事；本算子的价值在于"方位由数据给出"，
	///   适合反推未知排布。要按实际间距配对而非按方位，用 <c>FindNeighbors</c>（id 1624）。</para>
	///   <para><b>参数取向</b>返回 1 条 + <c>out</c> 3 条。两条序号按 INTEGER 装载，
	///   两条方位串未指定类型、按原生返回类型装载（实现里 <c>LoadNew(proc, 2/3, err, out ...)</c>），
	///   取用时按字符串处理。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("assembly.hobj");
	///   JlRegion bodies = image.Threshold(100.0, 255.0).Connection();
	///   JlRegion marks = image.Threshold(0.0, 40.0).Connection();
	///   JlTuple bodyIdx = bodies.SpatialRelation(marks, 50, out JlTuple markIdx,
	///       out JlTuple relation1, out JlTuple relation2);
	///   bodies.Dispose();
	///   marks.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>不产出句柄；两个 <c>Connection</c> 句柄要释放。坑：方位串是字符串元组，
	///   与序号元组长度一致才能逐位配对，若上游顺序在两次 <c>Connection</c> 之间发生变化，配对会静默错位 [待实测]。</para>
	/// </remarks>
	...
	/// <summary>逐区域的凹凸形状因子：越接近凸图形值越高，凹陷与缺口把它压低（DOUBLE 元组）。</summary>
	/// <returns>各区域的凸度（<c>double</c> 元组）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>只看外形"缺不缺肉"，与面积、朝向、位置无关：同样大小的齿轮和圆盘能靠它分开。
	///   确切定义（是否为区域面积与凸包面积之比）本层未注明 [待实测]，但它是归一后的比值量，
	///   不随区域整体缩放而变化。</para>
	///   <para><b>约束或前提</b>它是"区域 vs 自身凸包"的量，因此 <c>FillUp</c> 补掉内孔几乎不动它，
	///   而一次 <c>ErosionCircle</c> 把边缘啃出缺口就会明显压低——腐蚀后必须重算，
	///   别拿腐蚀前的凸度阈值去判腐蚀后的域。<c>Connection</c> 拆开的域逐位对齐，顺序变了值会张冠李戴。</para>
	///   <para><b>与相邻算子的取舍</b>要"真的拿到凸包区域"（后续做填充、比对）用
	///   <c>ShapeTrans("convex")</c>（id 498），本算子只给数；批量筛凸形用
	///   <c>SelectShape("convexity", ...)</c> 一次搞定，不必逐域比这个元组。要边缘毛糙程度用
	///   <c>Compactness</c> 或 <c>Roundness</c>：细密锯齿未必凹，凸度对它不敏感。</para>
	///   <para><b>参数取向</b>无入参、返回 1 条 DOUBLE 元组（原生 id 1643），本类无标量重载。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("gears.hobj");
	///   JlRegion gears = image.Threshold(80.0, 255.0).Connection();
	///   JlTuple convexity = gears.Convexity();
	///   gears.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>数值元组无需释放。<c>Connection</c> 的新句柄要释放。
	///   坑：区域只缺一个角时数值仍很高（比例量对局部缺陷不敏感），定位缺角应改用轮廓比对或
	///   <c>ShapeTrans</c> 差集，别指望本量报警。</para>
	/// </remarks>
	...
	/// <summary>区域边界长度（周长，DOUBLE 元组）：沿像素边界累加，不是一条拟合曲线的长度。</summary>
	/// <returns>各区域的边界长度（<c>double</c> 元组，像素）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>沿区域边界逐像素累加得到的长度，量的是"锯齿路径"而非理想几何周长：
	///   一个边长 100 像素的正方形只落在其理论周长约 400 的量级上（逐像素计法的偏差 [待实测]），斜边还会被 staircase 化拉长，
	///   因此别拿它当精确几何量用。</para>
	///   <para><b>约束或前提</b>边界按哪种邻域走（4 邻域步进还是 8 邻域含对角）直接改变数值，本层未注明 [待实测]；
	///   内孔的边界是否一并计入同样决定结果 [待实测]——带孔件判前要先确认口径。
	///   <c>Connection</c> 后逐域给值；单个像素或空洞区域给出什么 [待实测]。</para>
	...
	///   <para><b>参数取向</b>无入参、返回 1 条 DOUBLE 元组（原生 id 1644），本类无标量重载。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("yarn.hobj");
	///   JlRegion fibers = image.Threshold(70.0, 255.0).Connection();
	///   JlTuple contourLength = fibers.Contlength();
	///   fibers.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>数值元组无需释放。坑：一次 <c>DilationCircle</c> 会让细长毛刺并到主体上，
	///   边界长度骤降；想"平滑后再量"就先形态学再调本量，想"量出毛刺"就千万别先膨胀。</para>
	/// </remarks>
	...
	/// <summary>逐区域给出两件拓扑账：内部连通块数（返回值）与孔数（out），均为 INTEGER 元组。</summary>
	/// <param name="numHoles">各区域的孔数（INTEGER 元组）。</param>
	/// <returns>各区域的连通块数（INTEGER 元组）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>对句柄里每个区域元素分别数"它自己包含几块互相连着的像素、围住了几个洞"。
	///   两条元组与区域序号逐位对齐。欧拉数就是两者之差（见 <c>EulerNumber</c>，id 1638），
	///   需要拆开的两个数才用本算子。</para>
	///   <para><b>约束或前提</b>别把它和 <c>CountObj()</c> 混用：<c>CountObj()</c> 数的是句柄里的区域元素个数，
	///   而这里数的是每个元素内部的连通块数——若已经 <c>Connection</c> 拆过，返回值通常全为 1，
	///   再拿它当"块数"判据会永远得 1。孔的判定依赖像素邻域约定 [待实测]，
	...
	///   本算子只报数不产出区域。</para>
	///   <para><b>参数取向</b>返回 1 条 + <c>out</c> 1 条，原生出参槽 [0]=连通块数、[1]=孔数，
	///   都按 INTEGER 装载（id 1645）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("castings.hobj");
	///   JlRegion casting = image.Threshold(90.0, 255.0);
	///   JlTuple components = casting.ConnectAndHoles(out JlTuple holes);
	///   casting.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>两条数值元组无需释放。坑：未拆分的整图区域里，噪声点会被算成独立连通块，
	///   数值虚高；量孔数前先做一次 <c>OpeningCircle</c> 去掉散点，但开运算也可能把细颈夹断，两害相权。</para>
	/// </remarks>
	...
	/// <summary>连通块数与孔数（标量版）：只取句柄第 1 个区域元素的那一对计数。</summary>
	/// <param name="numHoles">输出：第 1 个区域的孔数。</param>
	/// <returns>第 1 个区域的连通块数。</returns>
	/// <remarks>
	...
	///   <c>Connection</c> 之后未排序时未必是你想要的那块 [待实测其取序依据]。
	...
	///   与本量的"连通块数"在已拆分句柄上恒为 1 的情况完全不同。</para>
	///   <para><b>参数取向</b>返回 1 标量 + <c>out</c> 1 标量。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlRegion plate = new JlRegion(20.0, 20.0, 120.0, 120.0);
	///   int components = plate.ConnectAndHoles(out int holes);
	///   plate.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>无分配。坑：按定义实心矩形是 <c>1 块 0 孔</c>；
	///   若句柄里塞了多个区域，两个数都只描述第 1 个，看起来"没问题"其实漏判。</para>
	/// </remarks>
	...
	/// <summary>逐区域的矩形度形状因子：形状越接近矩形值越高（DOUBLE 元组）。</summary>
	/// <returns>各区域的矩形度（<c>double</c> 元组）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>只问"像不像矩形"，不问大小也不问摆放角度：一个斜放的长方形与正放的
	///   长方形给出同量级的值。确切分母（最小外接斜矩形面积或其它归一量）本层未注明 [待实测]。</para>
	...
	///   用处理后的值配处理前的阈值会误判。</para>
	///   <para><b>与相邻算子的取舍</b>要矩形的具体位置与边长用 <c>SmallestRectangle1</c>（轴对齐）或
	///   <c>SmallestRectangle2</c>（带倾角），它们给几何不给"像不像"；要"真的把区域换成矩形"用
	///   <c>ShapeTrans</c>。判细长条用 <c>Eccentricity</c> 的长短轴比更直接，矩形度对细长条同样给高分。</para>
	///   <para><b>参数取向</b>无入参、返回 1 条 DOUBLE 元组（原生 id 1646），本类无标量重载。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("bricks.hobj");
	///   JlRegion bricks = image.Threshold(100.0, 255.0).Connection();
	///   JlTuple rectangularity = bricks.Rectangularity();
	///   bricks.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>数值元组无需释放。坑：圆角很小的近似矩形与标准矩形分数接近，
	///   想用本量卡"必须是直角"不现实，应转为 <c>SmallestRectangle2</c> 的面积差来验。</para>
	/// </remarks>
	...
	/// <summary>紧密度形状因子：把边界长度与面积合成一个与尺度无关的量（DOUBLE 元组）。</summary>
	/// <returns>各区域的紧密度（<c>double</c> 元组）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>周长与面积的组合归一量，因此对区域整体放大/缩小不敏感：
	///   大圆和小圆给同一个值，这是它比 <c>Contlength</c> 更好用于跨尺寸比较的原因。
	///   圆是该量的极值点，形状越碎越偏离；具体公式与"越大越碎还是越小越碎"的方向本层未注明 [待实测]。</para>
	...
	///   <para><b>与相邻算子的取舍</b>要"像不像圆"用 <c>Circularity</c>；要"凹凸缺口"用 <c>Convexity</c>；
	///   要原始周长用 <c>Contlength</c>。
	///   本量擅长的是"同样面积下谁的边更啰嗦"，不适合用来区分长宽比。</para>
	///   <para><b>参数取向</b>无入参、返回 1 条 DOUBLE 元组（原生 id 1647），本类无标量重载。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("powder.hobj");
	///   JlRegion grains = image.Threshold(110.0, 255.0).Connection();
	///   JlRegion smooth = grains.OpeningCircle(1.5);
	///   JlTuple compactness = smooth.Compactness();
	///   grains.Dispose();
	///   smooth.Dispose();
	///   </code>
	///   <para><b>资源与坑</b><c>Connection</c> 与 <c>OpeningCircle</c> 各返回新句柄，都要释放；
	///   数值元组免释放。坑：本量对面积接近 0 的碎屑不稳定（除以小面积），筛掉碎屑后再统计。</para>
	/// </remarks>
	...
	///   <c>SmallestCircle</c> 或 <c>InnerCircle</c>。</para>
	///   <para><b>参数取向</b>无入参、返回 1 条 DOUBLE 元组（原生 id 1648），本类无标量重载。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("balls.hobj");
	///   JlRegion balls = image.Threshold(120.0, 255.0).Connection();
	///   JlTuple circularity = balls.Circularity();
	///   balls.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>数值元组无需释放。坑：视野边缘被裁掉的圆，圆度会大跌——
	///   本量对"形状不完整"和"本来就不圆"不加区分，先按位置剔除裁边件再判。</para>
	/// </remarks>
	...
	/// <summary>逐区域内孔面积之和（INTEGER 元组，像素计数）：实心区域为 0。</summary>
	/// <returns>各区域的孔面积（<c>int</c> 元组）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>一个区域里所有被完全围住的背景像素总数。它给的是"孔有多大"，
	///   <c>ConnectAndHoles</c>（id 1645）给的是"有几个孔"，两者都要时才配着用。
	///   按定义，把孔填掉后的面积减去原面积应当等于本量（<c>FillUp</c> 与之对比可自校验）[待实测]。</para>
	///   <para><b>约束或前提</b>"孔"必须是闭合的：边缘有豁口时那块背景与外界相通，本量算 0，
	///   而人眼看是孔——这类误判最常见。像素连通性约定决定闭合判定 [待实测]。
	...
	///   再与 <c>Difference</c> 求差集；只要"有无孔"用 <c>EulerNumber</c> 更省。
	///   本量不区分是 1 个大孔还是 10 个小孔。</para>
	///   <para><b>参数取向</b>无入参、返回 1 条 INTEGER 元组（原生 id 1649），本类无标量重载。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("washers.hobj");
	///   JlRegion washers = image.Threshold(60.0, 255.0).Connection();
	///   JlTuple holeArea = washers.AreaHoles();
	///   washers.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>数值元组无需释放。坑：区域被视野边裁断时孔可能一并豁开，本量归零，
	///   判"无孔"前先确认区域没有贴边。</para>
	/// </remarks>
	...
	///   JlRegion disk = new JlRegion();
	///   disk.GenCircle(200.0, 200.0, 60.0);
	///   int area = disk.AreaCenter(out double row, out double column);   // 面积在返回值上
	///   disk.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>游程（run）长度分布：返回值给前景分布，out 给背景分布，两条 INTEGER 元组。</summary>
	/// <param name="background">背景游程长度分布（INTEGER 元组）。</param>
	/// <returns>前景游程长度分布（INTEGER 元组）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>把区域按行拆成一条条连续前景段（游程），统计其长度分布。
	///   元组里"下标即长度、值即出现次数"还是"逐条长度列表"，本层未注明 [待实测]——
	///   拿去做占空比或条纹判据前先打印一次核对结构。</para>
	///   <para><b>约束或前提</b>背景分布必须有明确的行范围才谈得上（行的起止、列的边界取自何处 [待实测]），
	///   因此同一区域在不同画幅下的背景分布不可比；前景分布只由区域自身决定。
	///   多区域句柄下的输出结构 [待实测]，静态版 <c>JlOperatorSet.RunlengthDistribution(JlObject, out JlTuple, out JlTuple)</c>
	///   接收的也是单个 region。</para>
	///   <para><b>与相邻算子的取舍</b>只要汇总指标（游程条数、平均长度、K/L 因子、字节数）用
	///   <c>RunlengthFeatures</c>（id 1623），一次拿四个数且更省；要整条长度分布（筛纹理、判条纹周期）
	///   才用本算子。要逐条拿到游程本身（行、起止列）用 <c>GetRegionRuns</c>，反向重建用
	///   <c>GenRegionRuns</c>；按游程长短去噪用 <c>EliminateRuns</c>。</para>
	///   <para><b>参数取向</b>返回 1 条 + <c>out</c> 1 条，两条均 INTEGER 装载（原生 id 1651）；
	///   <c>this</c> 进 iconic 槽 1。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("fabric.hobj");
	///   JlRegion weave = image.Threshold(100.0, 255.0);
	///   JlTuple foreground = weave.RunlengthDistribution(out JlTuple background);
	///   weave.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>两条数值元组无需释放。坑：<c>Connection</c> 会改变游程集合，
	///   拆域前后统计的是两码事，纹理判据要固定在同一种区域形态上取。</para>
	/// </remarks>
	...
	/// <summary>由边界到质心距离统计出的四件套：平均距离（返回值）、标准差、圆整度、等效边数。</summary>
	/// <param name="sigma">距离标准差（DOUBLE 元组）。</param>
	/// <param name="roundness">圆整度形状因子（DOUBLE 元组）。</param>
	/// <param name="sides">等效多边形边数（DOUBLE 元组）。</param>
	/// <returns>边界到中心的平均距离（DOUBLE 元组，像素）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>沿边界采样到中心的距离，再看这条距离曲线的统计量：<c>sigma</c> 越小越圆，
	///   <c>roundness</c> 是它的归一化形状因子（与平均距离之比一类的构造，确切公式 [待实测]），
	...
	///   距离曲线仍然良定义，但它与 <c>InnerCircle</c>、<c>SmallestCircle</c> 给出的圆心不是同一个点 [待实测]。</para>
	...
	///   <para><b>参数取向</b>返回 1 条 + <c>out</c> 3 条；原生出参槽 [0]=平均距离、[1]=sigma、
	///   [2]=roundness、[3]=sides（id 1652）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("nuts.hobj");
	///   JlRegion nuts = image.Threshold(90.0, 255.0).Connection();
	///   JlTuple meanDist = nuts.Roundness(out JlTuple sigma, out JlTuple roundness, out JlTuple sides);
	///   nuts.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>四条数值元组无需释放。坑：区域被视野裁掉一块时边界距离曲线出现台阶，
	///   <c>sides</c> 会给出无意义的多边形数，判边数前先剔除不完整域。</para>
	/// </remarks>
	...
	/// <summary>边界距离四件套（标量版）：只取句柄第 1 个区域的距离统计。</summary>
	/// <param name="sigma">输出：距离标准差。</param>
	/// <param name="roundness">输出：圆整度形状因子。</param>
	/// <param name="sides">输出：等效多边形边数（浮点，需自行取整）。</param>
	/// <returns>边界到参考中心的平均距离（像素）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>与元组版同一原生算子（id 1652），四次 <c>LoadD</c> 直取标量、
	///   不建 <c>JlTuple</c>；出参序仍是平均距离、sigma、roundness、sides。</para>
	///   <para><b>约束或前提</b>多区域句柄只反映第 1 个，其余静默丢弃；"第 1 个"取决于上游顺序，
	///   <c>Connection</c> 未配排序时不可依赖。逐域判边数请用元组版。</para>
	///   <para><b>与相邻算子的取舍</b>只要一个圆不圆的数用 <c>Circularity()</c>；
	///   本重载的用处是"单件检测"时少建四条元组。<c>sides</c> 是 <c>double</c>，
	///   拿它当数组下标或做等值比较都需先归整。</para>
	///   <para><b>参数取向</b>返回 1 标量 + <c>out</c> 3 标量。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlRegion nut = new JlRegion(40.0, 40.0, 80.0, 80.0);
	///   double meanDist = nut.Roundness(out double sigma, out double roundness, out double sides);
	///   nut.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>无句柄、无元组分配。坑：轴对齐矩形这类"人造区域"边界规则，
	///   <c>sides</c> 会给出接近 4 的读数 [待实测]，别把它当成实测结论用。</para>
	/// </remarks>
	...
	/// <summary>区域内最大轴对齐内接矩形：四个 <c>out</c> 给出左上、右下角点（INTEGER 元组）。</summary>
	/// <param name="row1">左上角行坐标（INTEGER 元组）。</param>
	/// <param name="column1">左上角列坐标（INTEGER 元组）。</param>
	/// <param name="row2">右下角行坐标（INTEGER 元组）。</param>
	/// <param name="column2">右下角列坐标（INTEGER 元组）。</param>
	/// <remarks>
	///   <para><b>它量什么</b>在区域内部塞得下的最大轴对齐矩形（按面积计），四个坐标与
	///   <c>JlRegion(double,double,double,double)</c> 的角点约定一致：<c>(row1,column1)</c> 左上、
	///   <c>(row2,column2)</c> 右下，行向下、列向右增大。它总是落在区域内部，
	...
	///   多个同面积解如何挑 [待实测]；空区域的输出 [待实测]。逐区域对齐，
	...
	///   要外轮廓包紧一点用 <c>SmallestRectangle2</c>。</para>
	///   <para><b>参数取向</b><c>void</c> 返回，四路 <c>out</c>：槽序 row1、column1、row2、column2（id 1653），
	///   与形参顺序一致；先声明 <c>out</c> 再接值，漏写 <c>out</c> 编译不过。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("panels.hobj");
	///   JlRegion panel = image.Threshold(100.0, 255.0);
	///   panel.InnerRectangle1(out JlTuple row1, out JlTuple column1, out JlTuple row2, out JlTuple column2);
	///   panel.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>四条整数元组无需释放。坑：拿这四个值直接 <c>new JlRegion(...)</c> 时
	///   端点是否闭合决定差一个像素 [待实测]，与 <c>GetRegionRuns</c> 的端点约定一样要核对一次。</para>
	/// </remarks>
	...
	/// <summary>最大轴对齐内接矩形（标量版）：只取句柄第 1 个区域的四个角点坐标。</summary>
	/// <param name="row1">输出：左上角行坐标。</param>
	/// <param name="column1">输出：左上角列坐标。</param>
	/// <param name="row2">输出：右下角行坐标。</param>
	/// <param name="column2">输出：右下角列坐标。</param>
	...
	///   逐域要矩形请用元组版。矩形是像素级整数格点，没有亚像素分辨率。空区域时四个值的取值 [待实测]。</para>
	...
	///   <para><b>参数取向</b><c>void</c> 返回 + 四路 <c>out int</c>，四个都得写 <c>out</c>。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlRegion plate = new JlRegion(10.0, 10.0, 90.0, 160.0);
	///   plate.InnerRectangle1(out int row1, out int column1, out int row2, out int column2);
	///   int width = column2 - column1;
	///   int height = row2 - row1;
	///   plate.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>无分配。坑：宽、高由角点相减时是否 +1 取决于端点闭合约定 [待实测]，
	...
	///   <para><b>用法</b></para>
	///   <code>
	///   JlRegion blob = new JlRegion(50.0, 30.0, 150.0, 200.0);
	///   blob.InnerCircle(out double row, out double column, out double radius);
	///   JlRegion core = new JlRegion();
	///   core.GenCircle(row, column, radius);   // 内切圆给出的是区域内最厚实的一坨
	///   core.Dispose();
	///   blob.Dispose();
	///   </code>
	/// </remarks>
	...
	/// <summary>沿全部灰度阈值扫描：逐阈值取"包含给定点的那块区域"算特征，给出绝对与相对两条直方图。</summary>
	/// <param name="image">参与阈值扫描的灰度图像。</param>
	/// <param name="feature">要统计的特征名。Default: "convexity"</param>
	/// <param name="row">必须被跟踪区域包含的像素行坐标。Default: 256</param>
	/// <param name="column">必须被跟踪区域包含的像素列坐标。Default: 256</param>
	/// <param name="relativeHisto">相对分布（DOUBLE 元组）。</param>
	/// <returns>绝对分布（逐阈值的目标特征值元组）。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>对每个候选阈值，取图像二值化后"包住 <c>(row,column)</c> 这一点"的那块区域，
	...
	///   否则该阈值下无区域可跟，输出会短一截或为空 [待实测]。两条直方图的长度等于实际参与统计的阈值个数，
	///   阈值序列的起止与步长由原生侧决定 [待实测]，因此别按 256 硬算下标。
	///   传入的区域是"参考区域"，本算子不改写它。</para>
	///   <para><b>与相邻算子的取舍</b>不关心某一点、要把整幅图各阈值的全局统计一次拿全，用
	///   <c>ShapeHistoAll</c>（id 1667，少两个坐标参数）；已经定好阈值只想看一个值，
	///   直接 <c>Threshold</c> 后调 <c>Convexity</c> 等量测即可，不必扫全曲线。</para>
	///   <para><b>参数取向</b>5 个实参（图像是第 2 路输入），<c>row</c>/<c>column</c> 以 <c>StoreI</c>
	///   直写、<c>feature</c> 以 <c>StoreS</c> 直写，无钉固定元组；返回绝对分布、
	///   相对分布走 <c>out</c>（原生 id 1666）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("tablet.hobj");
	///   JlRegion roi = image.Threshold(0.0, 255.0);
	///   JlTuple absolute = roi.ShapeHistoPoint(image, "convexity", 125, 155, out JlTuple relative);
	///   roi.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>两条输出元组无需释放；<c>Threshold</c> 得到的 <c>roi</c> 是新句柄要释放。
	///   坑：多通道图像不能直接喂，需先取单通道；<c>image</c> 与区域必须同幅面同尺度，
	///   经过 <c>ZoomRegion</c> 之类的变换后坐标点要一并换算，否则跟错目标。</para>
	/// </remarks>
	...
	/// <summary>沿全部灰度阈值扫描整幅特征分布：不需跟踪点，返回绝对分布、相对分布走 out。</summary>
	/// <param name="image">参与阈值扫描的灰度图像。</param>
	/// <param name="feature">要统计的特征名。Default: "connected_components"</param>
	/// <param name="relativeHisto">相对分布（DOUBLE 元组，按各阈值归一）。</param>
	/// <returns>绝对分布（逐阈值的特征值元组）。</returns>
	/// <remarks>
	...
	///   <para><b>约束或前提</b>本句柄区域充当统计 ROI，区域外的像素不参与；传空区域时的行为 [待实测]。
	///   阈值序列长度与起止由原生侧定 [待实测]，两条元组等长、按位对应。多通道图像不适用，需先取单通道。</para>
	...
	///   <para><b>参数取向</b>3 个实参（图像为第 2 路输入）；<c>feature</c> 以 <c>StoreS</c> 直写控制参数 0，
	///   返回绝对分布（未显式指定类型，按原生返回装载），相对分布按 DOUBLE 装载（id 1667）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("tablets.hobj");
	///   JlRegion full = image.Threshold(0.0, 255.0);
	///   JlTuple absolute = full.ShapeHistoAll(image, "connected_components", out JlTuple relative);
	///   full.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>两条元组无需释放，本算子不产出句柄。坑：块数曲线在低阈值端常因背景粘连
	///   而剧烈抖动，取平台段时要跳过两端，别把抖区当最优阈值。</para>
	/// </remarks>
	...
	/// <summary>在每个区域上量灰度特征：一次传一组特征名，返回全部数值（DOUBLE 元组）。</summary>
	/// <param name="image">提供灰度的图像，必须与区域同幅面。</param>
	/// <param name="features">特征名列表（<c>JlTuple</c>，可一次给多个）。Default: "mean"</param>
	/// <returns>各区域各特征的取值，统一按 DOUBLE 装载成一条元组。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>把 <c>this</c> 里每个区域当作 ROI，在 <c>image</c> 上算 <c>features</c>
	///   列出的灰度量（均值、偏差一类，可用名字清单本层未枚举 [待实测]）。
	///   所有值都按 DOUBLE 装载，连"像素计数"类特征也是浮点，取整需自己转。</para>
	///   <para><b>约束或前提</b>返回元组是"区域 × 特征"的展开，主次顺序（先遍历区域还是先遍历特征）
	///   本层未注明 [待实测]，多特征时务必先用单特征调用核对一次排布，否则按下标取值会静默拿错列。
	...
	///   本算子的用处是"一次取多特征再自己定判据"。</para>
	///   <para><b>参数取向</b>2 个实参（<c>this</c> 为第 1 路、图像为第 2 路）。<c>features</c> 以
	///   <c>Store</c> 钉固定元组、调用后 <c>UnpinTuple</c>；只查一个特征时用 <c>GrayFeatures(JlImage, string)</c>
	///   可省这次固定（原生 id 1668）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("coins.hobj");
	///   JlRegion coins = image.Threshold(100.0, 255.0).Connection();
	///   JlTuple values = coins.GrayFeatures(image, new JlTuple("mean", "deviation"));
	///   coins.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>返回元组免释放；<c>Connection</c> 的新句柄要释放。坑：<c>image</c> 与
	///   区域在原生调用结束前不得释放（实现用 <c>GC.KeepAlive</c> 保证），不要在调用前后提前 Dispose。</para>
	/// </remarks>
	...
	/// <summary>灰度特征（标量版）：单特征名单值查询，只取返回元组的第 1 个值。</summary>
	/// <param name="image">提供灰度的图像，需与区域同幅面。</param>
	/// <param name="features">单个特征名。Default: "mean"</param>
	/// <returns>第 1 个区域的该特征值。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>与元组版同一原生算子（id 1668）：<c>StoreS</c> 直写单个特征名，
	///   结果用 <c>LoadD</c> 只读第 1 个值，不建 <c>JlTuple</c>、无钉固定/解钉开销。</para>
	///   <para><b>约束或前提</b>这是本重载最需要注意的一处：句柄里有多个区域时，
	///   只有第 1 个区域的值被取出，其余静默丢弃；而"第 1 个"取决于 <c>Connection</c> 的输出顺序。
	///   逐域取值请用 <c>GrayFeatures(JlImage, JlTuple)</c>。</para>
	///   <para><b>与相邻算子的取舍</b>要按阈值留区域用 <c>SelectGray</c>；要同时拿均值和偏差用
	///   <c>Intensity(JlImage, out double)</c>。本重载适合"确认单个目标的一维指标"这类点查。</para>
	///   <para><b>参数取向</b>2 个实参（<c>this</c> 第 1 路、图像第 2 路），返回 1 个 <c>double</c>。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("label.hobj");
	///   JlRegion patch = image.Threshold(120.0, 255.0);
	///   double mean = patch.GrayFeatures(image, "mean");
	///   patch.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>无句柄无元组分配。坑：特征名拼错时是否报错由原生侧决定 [待实测]，
	...
	/// <summary>按灰度特征筛区域：逐区域算特征再比区间，命中的组成一个新区域句柄。</summary>
	/// <param name="image">提供灰度的图像，需与区域同幅面。</param>
	/// <param name="features">特征名列表，与 <c>min</c>/<c>max</c> 一一对应。Default: "mean"</param>
	/// <param name="operation">多个特征之间的逻辑连接方式。Default: "and"</param>
	/// <param name="min">各特征下限，也可写成表示"不设下限"的边界标记。Default: 128.0</param>
	/// <param name="max">各特征上限，写法同上。Default: 255.0</param>
	/// <returns>新 <c>JlRegion</c>：特征落在区间内的区域集合，用毕需释放。</returns>
	/// <remarks>
	...
	///   <para><b>约束或前提</b>三个元组参数须与 <c>features</c> 等长并按位对应，长度不匹配时的行为 [待实测]。
	///   <c>operation</c> 为 "and" 时全部特征都要满足，"or" 时任一满足即可（其余取值 [待实测]）。
	///   英文文档提到边界可写 'min'/'max' 表示该端不限，本重载参数是 <c>JlTuple</c> 才容得下字符串写法，
	///   原生侧是否接受 [待实测]。区间开闭 [待实测]。</para>
	...
	///   <c>features</c>/<c>min</c>/<c>max</c> 走 <c>Store</c> 钉固定、调用后 <c>UnpinTuple</c>；
	///   输出经 <c>LoadNew</c> 装在 iconic 槽 1，是新句柄而非原地改写，原句柄不变。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("stamps.hobj");
	///   JlRegion all = image.Threshold(90.0, 255.0).Connection();
	///   JlRegion bright = all.SelectGray(image, new JlTuple("mean"), "and",
	///       new JlTuple(150.0), new JlTuple(255.0));
	///   int kept = bright.CountObj();
	///   all.Dispose();
	///   bright.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>返回值是新句柄必须释放，<c>Connection</c> 得到的 <c>all</c> 同样要释放。
	///   坑：筛后序号相对原句柄被压缩，之后再拿原句柄的序号去 <c>SelectObj</c> 会错位；
	///   需要保留对应关系时改用 <c>GrayFeatures</c> 自己取序号。</para>
	/// </remarks>
	...
	/// <summary>按单个灰度特征筛区域（标量版）：区间两端必须给数值，命中的组成新句柄。</summary>
	/// <param name="image">提供灰度的图像，需与区域同幅面。</param>
	/// <param name="features">单个特征名。Default: "mean"</param>
	/// <param name="operation">多特征时的逻辑连接（本重载只有一个特征，实际不起作用）。Default: "and"</param>
	/// <param name="min">下限数值。Default: 128.0</param>
	/// <param name="max">上限数值。Default: 255.0</param>
	/// <returns>新 <c>JlRegion</c>：特征在区间内的区域集合，用毕需释放。</returns>
	/// <remarks>
	///   <para><b>它量什么</b>与元组版同一原生算子（id 1669）：<c>StoreS</c>/<c>StoreD</c> 直写标量，
	...
	///   单特征下 <c>operation</c> 无实际作用，但仍是必填实参。区间开闭 [待实测]。</para>
	...
	///   using JLVisionLib;
	///
	///   JlImage image = new JlImage("bottles.hobj");
	///   JlRegion caps = image.Threshold(80.0, 255.0).Connection();
	///   JlRegion darkCaps = caps.SelectGray(image, "mean", "and", 0.0, 120.0);
	///   caps.Dispose();
	///   darkCaps.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>返回值与 <c>Connection</c> 的结果都是新句柄，都要释放。坑：<c>min &gt; max</c>
	///   时的行为（报错还是返回空）本层未注明 [待实测]，写反区间容易悄悄筛空。</para>
	/// </remarks>
	...
	///   <para><b>用法</b></para>
	///   <code>
	///   JlImage image = new JlImage("grad.hobj");
	///   JlRegion roi = new JlRegion(50.0, 50.0, 200.0, 260.0);
	///   double volume = roi.AreaCenterGray(image, out double row, out double column);
	///   roi.Dispose();
	///   image.Dispose();
	///   </code>
	/// </remarks>
