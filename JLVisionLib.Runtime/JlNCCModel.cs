...
{
	/// <summary>
	///   框架与占位用的纯托管空壳构造（带 EditorBrowsable(Never)）：基类句柄值置 UNDEF（IntPtr.Zero），不发任何原生调用，IsInitialized() 为 false，把它直接送进算子会由 PostCall 报错。它是 LoadNew、Clone、静态 Deserialize 与 ISerializable 构造共用的"先建壳再原地装载"起点；业务取可用模型应走文件名构造（原生 id 939）、模板图训练构造（id 947）或 Clone，且训练后再改生成参数（SetNccModelParam，id 946）需重新训练模型方可生效。
	/// </summary>
	/// <remarks>
	///   <para><b>功能说明</b>: base(JlHandleBase.UNDEF) 落到句柄赋值的 UNDEF 分支——跳过原生 CopyHandle，只建一个待装载的托管壳，零原生开销。</para>
	///   <para><b>约束或前提</b>空壳不是无效对象：正好充当 DeserializeNccModel（id 937）等原地装载通道的接收位（基类 Load 要求装载前当前值必须是 UNDEF）；用完同样要 Dispose 做托管簿记收尾。</para>
	///   <para><b>与相邻构造器的取舍</b>从文件取模型用 JlNCCModel(string fileName)，从图像训练用带模板图的构造器，手里已有原生句柄用 IntPtr 或 JlHandle 包装构造。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   JlNCCModel shell = new JlNCCModel();    // 空壳：无原生模型句柄
	///   bool ready = shell.IsInitialized();     // false
	///   shell.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>本构造与其余三个构造器都标 EditorBrowsable(Never)，意在让常规补全只暴露训练/读文件路径；空壳若忘记装载就当模型用，第一声报错来自原生调用而非构造函数。</para>
	/// </remarks>
	...
	/// <summary>
	///   包装外部裸句柄值的浅拷贝通道（带 EditorBrowsable(Never)）：经基类 Handle 赋值走原生 CopyHandle 另取一份引用——拿到的是同一原生 ncc_model 的第二个名字，不接管原指针所有权，两壳各释各的引用；句柄语义类型必须为 ncc_model，不符抛 JlException("Invalid handle instance passed")，传 IntPtr.Zero（UNDEF）则跳过拷贝与类型校验、直接得空壳；悬垂句柄值在构造阶段就抛 JlOperatorException，不会静默退化成空壳。
	/// </summary>
	...
	///   <para><b>用法</b></para>
	///   <code>
	///   using JlNCCModel model = new JlNCCModel("bottle.ncm");  // 从模型文件读入（id 939）
	///   IntPtr raw = model.Handle;                              // 裸句柄值
	///   using JlNCCModel alias = new JlNCCModel(raw);           // 再取一份引用，两个 C# 壳独立
	///   </code>
	///   <para><b>资源与坑</b>存 raw 这种裸值不带引用计数：model 一 Dispose，raw 就成了看着像有效（非 0）的悬垂值，再喂给本构造会在原生拷贝处抛错——这是最难查的一类用法。</para>
	/// </remarks>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>
	///   包装任意 JlHandle 派生对象的拷贝构造（带 EditorBrowsable(Never)）：与 IntPtr 版同一基类路径——CopyHandle 引用计数拷贝，C# 壳独立、各须各 Dispose，但不是深拷贝；构造末尾按语义类型 ncc_model 校验，源句柄是 image/region 等其他类型即抛 JlException("Invalid handle instance passed")；实参传 null 不抛空引用，经 IntPtr 隐式转换归一为 UNDEF、得到空壳且跳过类型校验。
	/// </summary>
	...
	///   <para><b>用法</b></para>
	///   <code>
	///   JlHandle src = new JlNCCModel("bottle.ncm");   // 静态类型 JlHandle、运行时语义 ncc_model
	///   JlNCCModel model = new JlNCCModel(src);        // 引用拷贝包装；src 换非 ncc_model 句柄即抛 JlException
	///   model.Dispose();
	///   src.Dispose();
	///   </code>
	///   <para><b>资源与坑</b>两个壳谁先释放都行（各还各的引用），但都释放完之前别把 src 当"已被 model 接管"来置 null——model 只是多了一个名字。</para>
	/// </remarks>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>
	///   从模型文件读入一个已训练好的 NCC 灰度相关模型，构造并持有一个新的原生句柄对象（读回即可直接 FindNccModel，无需重建）。
	/// </summary>
	...
	/// <summary>
	///   用模板图像 domain 内的灰度训练一个 NCC 灰度相关匹配模板（角度以弧度计、numLevels/angleStep 可传 "auto"），返回持有新原生句柄的模型对象。
	/// </summary>
	...
	/// <summary>
	///   以标量金字塔层数与弧度角步长训练 NCC 灰度相关模板（内容取模板 domain 灰度），返回持有新原生句柄的模型对象；本重载 numLevels/angleStep 不能传 "auto"。
	/// </summary>
	...
	/// <remarks>
	///   <para><b>功能说明</b>方法体一句 DeserializeNccModel((byte[])info.GetValue("data", typeof(byte[])))：字节先包成 JlSerializationBuffer（serialized_item 句柄）再喂原生 id 937，装载结果写回本实例——构造完成即持有新句柄，无返回值形态问题。</para>
	///   <para><b>约束或前提</b>与本类 GetObjectData（同样键名 "data"）配对；训练后再改生成参数需重新训练方可生效的约定对反序列化还原的模型同样成立。</para>
	///   <para><b>与相邻构造器的取舍</b>手上有流用静态 Deserialize(Stream)（内部即空壳加 DeserializeNccModel，与本构造等价）；内存字节直接 new 空壳后调 DeserializeNccModel；落盘文件用文件名构造（id 939）。</para>
	///   <para><b>用法</b></para>
	///   <code>
	///   using (JlNCCModel src = new JlNCCModel("bottle.ncm"))
	///   {
	///       byte[] blob = src.SerializeNccModel();     // id 938：模型转字节数组
	///       using JlNCCModel dst = new JlNCCModel();   // 空壳：与 ISerializable 构造同一起点
	///       dst.DeserializeNccModel(blob);             // id 937：该构造体的同一条原地装载通道
	///       bool loaded = dst.IsInitialized();         // true
	///   }
	///   </code>
	///   <para><b>资源与坑</b>DeserializeNccModel 是原地替换语义：在已有内容的模型上重放会先释放旧句柄，想保留原模型必须先 new 空壳再装载。</para>
	/// </remarks>
	[EditorBrowsable(EditorBrowsableState.Never)]
	...
	/// <summary>把本 NCC 模型序列化的字节流写入给定 Stream（Vision 二进制格式，内部先调 SerializeNccModel 再落流），只读、不改动也不释放本对象句柄。</summary>
	/// <remarks>
	...
	/// <summary>静态：从流读出数据还原一个 NCC 模型，返回持有全新原生句柄的 JlNCCModel 对象，不修改任何既有对象。</summary>
	/// <remarks>
	...
	/// <summary>深拷贝整个 NCC 模型，返回持有独立新句柄、含已训练模板数据的副本。</summary>
	/// <remarks>
	...
	/// <summary>
	///   释放本 NCC 模型在原生侧占用的内存（只调原生删除、不清空托管句柄字段），无返回值；调用后该对象不应再用于匹配。
	/// </summary>
	/// <remarks>
	...
	/// <summary>
	///   从字节数组原地还原并替换本 NCC 模型（先释放旧句柄再把新句柄装入本对象），不返回新对象。
	/// </summary>
	...
	/// <summary>
	///   把本 NCC 模型序列化为 Vision 二进制格式的 byte[] 并返回，只读、不改动本对象句柄，调用后模型仍可继续匹配。
	/// </summary>
	...
	/// <summary>
	///   从模型文件原地读取并替换本 NCC 模型（先释放旧句柄再把文件内容的新句柄装入本对象），不返回新对象；构造器 JlNCCModel(fileName) 是同算子的新对象版本。
	/// </summary>
	...
	/// <summary>
	///   把本 NCC 模型以 Vision 二进制格式写入指定文件（存的是训练完成的模型本体而非图像），只读、不改动本对象句柄，无返回值。
	/// </summary>
	...
	/// <summary>静态：不建模、不产生句柄，依模板 domain 灰度统计推荐参数，返回被确定的参数名元组并经 out 给出等长按序对应的 DOUBLE 域推荐值。</summary>
	...
	/// <summary>静态标量重载：numLevels、parameters 直写标量，同不建模、不产生句柄，返回被确定的参数名元组并经 out 给出等长按序对应的推荐值。</summary>
	...
	/// <summary>
	///   回读本 NCC 模型已固化的实际参数：三个弧度角与生效匹配度量经 out 给出，返回值即金字塔层数（INTEGER，非错误码）；只读、不改句柄。
	/// </summary>
	...
	/// <summary>
	///   取本 NCC 模型参考点的像素坐标（row=y、column=x，即 FindNccModel 所报告的定位点），经两个 out 返回，只读、不改句柄。
	/// </summary>
	...
	/// <summary>
	///   原地改设本 NCC 模型的参考点像素坐标（模型内部坐标，非图像坐标），此后 FindNccModel 的 row/column 立即按新参考点报告；只改参考点、不动灰度数据、不换句柄，无返回值。
	/// </summary>
	...
	/// <summary>
	///   FindNccModel 的标量重载（numLevels 直写整数、无法表达负数二元组编码）：按 minScore 过滤后取前 numMatches 个 NCC 匹配实例，把各实例参考点行/列（像素）、弧度角与得分经四个等长 DOUBLE out 元组给出，无返回值。
	/// </summary>
	...
	/// <summary>
	///   在既有 JlNCCModel 对象上原地重建 NCC 灰度相关模板（取模板 domain 灰度、角度以弧度，numLevels/angleStep 可 "auto"）：先释放旧句柄再装入新模型句柄，无返回值。
	/// </summary>
	/// <param name="template">模板图像；模型内容取其 domain 内灰度。Input image whose domain will be used to create the model.</param>
	...
	/// <summary>
	///   CreateNccModel 的标量重载：在既有对象上原地重建 NCC 模板（numLevels/angleStep 为整数/弧度标量、不能传 "auto"），先释放旧句柄再装入新模型，无返回值。
	/// </summary>
	/// <param name="template">模板图像（取 domain 内灰度）。Input image whose domain will be used to create the model.</param>
	...
	/// <summary>
	///   以本对象为唯一模型走批量算子搜 NCC 实例（参数以标量直写），把各实例参考点行/列（像素）、弧度角、得分与模型索引经五个等长 out 元组给出；numMatches=0 时全取、输出长度不定，无返回值。
	/// </summary>
	/// <param name="image">待搜图。Input image in which the model should be found.</param>
	...
	/// <summary>
	///   取回建模时由模板 domain 生成的模型区域（哪些像素真正进了模板），返回一个新的 JlRegion 句柄，只读、不改本模型。
	/// </summary>
